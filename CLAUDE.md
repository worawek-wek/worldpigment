...
- 🔴 **รหัสผ่าน = รหัสผ่านของบัญชีที่ล็อกอินอยู่** (ผู้ใช้สั่ง 12/09/2569 — ตอนแรกทำเป็นรหัสร่วม `config('order.md_password')` แบบฟอร์ม MK แล้วเปลี่ยน): พนักงาน guard `emp` → `emp.password` · admin guard `web` → `users.password` · อ่านผ่าน `AccessControl::currentAccount()->getAuthPassword()` แล้ว `Hash::check` (ตัวเดียวกับตอน login)
- ข้อความ 422 แยก 3 กรณี: ไม่พบบัญชีที่ล็อกอิน · **บัญชีนี้ยังไม่ได้ตั้งรหัสผ่าน** · รหัสผ่านไม่ถูกต้อง · ⚠ `emp` มี 2 คอลัมน์รหัสผ่าน — ใช้ `password` (hash) ไม่ใช่ `pwd` ของระบบเก่า (varchar 10) · **มีพนักงานตั้งรหัสไว้แค่ 5 จาก 33 คน** (ที่เหลือล็อกอินไม่ได้อยู่แล้ว)
