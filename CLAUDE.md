...
- **หน้าเดียว 2 มุมมอง**: แท็บ **ตารางวันหยุด** (DataTables serverSide, กรอง ปี/ประเภท/สถานะ + ค้นหาชื่อ-หมายเหตุ, sort ได้ทุกคอลัมน์ที่เป็นคอลัมน์จริง) และแท็บ **ปฏิทินรายปี** (12 เดือน, วันหยุดเป็นวงกลมสีตามประเภท, **คลิกวันที่ = เปิดฟอร์มแก้ไข** เพราะใช้ class `btn_edit` ตัวเดียวกับปุ่มในตาราง)
  - ปฏิทินโหลดผ่าน AJAX (`holiday.calendar` → partial `holiday/calendar.blade.php`) **ตอนเปิดแท็บครั้งแรกเท่านั้น** — บันทึก/ลบ/สลับสถานะจะสั่งโหลดใหม่ถ้าแท็บเปิดอยู่ ไม่งั้นตั้งธง `calendarLoaded=false` ไว้โหลดตอนเปิดแท็บ
