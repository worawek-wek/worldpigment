...
- **กรอง:** `Appv` ≠ 0 และไม่ null (Access เก็บ -1 = อนุมัติแล้ว) + `ReqDate >= วันนี้ 00:00 - 2 วัน`
- ⚠ **"เวลาอนุมัติ" ใช้ค่าเดียวกับ "วันที่ขอ" (`ReqDate`)** — ตาราง `appvreq` มีแค่ `ReqDate`/`Appv` **ไม่มีคอลัมน์เวลาที่กดอนุมัติ** (ต่างจาก `morder` ที่มี `appv` + `appvDT`) · ผู้ใช้เลือกแนวทางนี้แทนการเพิ่มคอลัมน์ (12/09/2569) — ถ้าภายหลังเพิ่ม `appvreq.appvDT` ให้แก้แค่บรรทัด `$r->appv_at = ...` ในเมธอด แล้วเปลี่ยนคอลัมน์ที่ใช้กรองช่วงวัน
- ⚠ **"3 วันล่าสุด" = 3 วันปฏิทินย้อนหลัง** (วันนี้ + 2 วันก่อนหน้า, ค่าคงที่ `PRINT_RECENT_DAYS`) ตามที่ผู้ใช้เลือก ⇒ ช่วงไหนไม่มีใครอนุมัติ รายงานจะขึ้น "ไม่พบรายการที่อนุมัติราคาใน 3 วันล่าสุด" · **ในฐานข้อมูล dev ปัจจุบัน `ReqDate` ล่าสุดคือ 25/08/2026 ⇒ กดพิมพ์แล้วจะได้หน้าว่างเป็นเรื่องปกติ ไม่ใช่บั๊ก**
