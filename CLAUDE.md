...
- **แต่ละป้ายโชว์ "ราคาของกลุ่มนั้น" บรรทัดล่างด้วย (19/09/2569 ตามที่ผู้ใช้สั่ง)** — โครงป้ายเปลี่ยนจาก `<div>` ข้อความล้วน เป็น 2 `<span>`: `.pa-gitem-cap` (ข้อความเกณฑ์เดิม) + **`.pa-gitem-price` (`#a_gprice1/2/3`)**
  ...
  - 🟢 **ยกเว้นรหัสที่มีในกลุ่มราคา (`zcolorrate`) → ป้ายโชว์ `rate_A` / `rate_B` / `rate_C` แทน** (26/09/2569 — ผู้ใช้แจ้งว่าป้ายไม่ตรงกับเมนู "ดูกลุ่มราคา") · `PriceApprovalController::data()` แนบ key **`color_rate`** (null เมื่อรหัสไม่มีในตาราง) → `fillApprovalForm()` เรียก `setApprovalGroupPrices()` ทับหลัง `fillApprovalPrices()` · **ช่องราคา 3 ช่องด้านบนยังเป็นราคาขาย 1/2/3 เหมือนเดิม** (แก้เฉพาะป้าย)
