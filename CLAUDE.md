...
│   │   ├── ColorRateController.php   # จัดการกลุ่มราคา zcolorrate — ราคาขั้นต่ำกลุ่ม A/B/C (19/09/2569)
...
│   │   ├── QcStatusController.php   # สถานะ QC tb_qc_status — CRUD master (09/09/2569)
...
│   └── colorrate.php            # จัดการกลุ่มราคา zcolorrate (19/09/2569)
└── public/
```
...
- `color-matching.php`, `quotation.php`, `saleinfo.php`, `pricerule.php`, `colorrate.php`, `order.php`, `customer.php`, `report.php`, `permission.php`, `product.php`, `holiday.php`, `worker.php`, `production.php`
...
- **หน้าพนักงานหน้างาน (Worker portal)** (`WorkerPlanningController`, `routes/worker.php` prefix `worker`, layout สลิม `layout/worker` ไม่มี sidebar/เมนู, 11/08/2569): ให้ emp ที่ role = **"Worker"** (`Emp::WORKER_ROLE_NAME` + `Emp::isWorker()`) อัพเดทสถานะการผลิต **เฉพาะงานของตัวเอง** (`tb_planning.empno` = ผู้ล็อกอิน). login ใช้หน้าเดิม (guard `emp`) → `AccessControl::homeUrl()` พา Worker ไป `worker.planning.index`. **route ไม่อยู่ใน `config/menu.php`** จึง pass-through `CheckAccess` → ต้องมี middleware **`worker`** (`WorkerOnly`, ขึ้นทะเบียนใน Kernel) กันบัญชีอื่น + ทุก query/update **กรอง+ตรวจ `empno` ที่ server เสมอ** (`ownJobOrFail`). ค้นหารวมช่องเดียว (`red_bill_code`/`itemno`/`machine_no` LIKE) + กรอง `inplan`. อัพเดทแตะเฉพาะ `planning_status` (เลือกได้ทุกสถานะของแผนกงานนั้น จาก `tb_planning_status` โดย map `company`→`dept_id`) + validate ว่าสถานะอยู่ในแผนก + บันทึกประวัติทุกครั้งลง **`tb_planning_status_log`** (`old/new_status`, `changed_by`=empno, `changed_at`). ปุ่ม "ดูรายละเอียด" เป็น modal อ่านอย่างเดียว
  - **ตัดงานที่ปิดแล้วออก + บล็อกฝั่ง server (09/09/2569):** ทั้งการดึงตารางและการอัพเดทสถานะ ไม่รวมงานที่ปิดแล้ว — ปิดงานราย item (`tb_planning.end_job='Y'`) **หรือ** ปิดออเดอร์ทั้ง header (`tb_planning_header.end_order='Y'`). (1) `datatable()` เพิ่ม 2 เงื่อนไข where แต่ละตัวครอบ `->orWhereNull(...)` ให้ NULL นับเป็น "ยังไม่ปิด" (ตามแพทเทิร์นรายงานการขาดวัตถุดิบ — `NULL != 'Y'` ใน MySQL ให้ผล NULL ไม่ใช่ true จึงต้อง orWhereNull ไม่งั้นแถว NULL หลุดหาย). (2) **บล็อกฝั่ง server** ที่ `statusUpdate()` — helper `jobIsClosed()` เช็คจากค่าจริงใน DB (ผ่าน `ownJobOrFail` ที่ eager-load `planning_header` อยู่แล้ว) ถ้าปิดแล้วตอบ **422 "งานนี้ถูกปิดแล้ว…"** พร้อมธง **`job_closed: true`** ก่อนแตะ DB — กัน bypass ฝั่ง client (ยิงตรง endpoint / เปิด modal ค้างไว้แล้วงานถูกปิดทีหลัง). การกรองตารางแค่ "ซ่อนจากตา" ด่าน server จึงจำเป็นเพราะ `statusUpdate` เป็นคนละ endpoint กับตาราง. **ฝั่ง JS (`worker/planning/index.blade.php` `submitStatus`):** error handler เช็ค `res.job_closed` — ถ้าจริงให้ **ปิด modal + reload ตาราง** (`loadJobs()`) แล้วเด้ง Swal แบบ warning (งานหลุดจากตารางไปแล้ว) แยกจาก 422 อื่น (เลือกสถานะผิดแผนก ฯลฯ) ที่โชว์ error เฉย ๆ ไม่ปิด modal
  - **ตั้ง qc_date + qc_time เมื่อเข้าสถานะ "ส่ง QC รอผล" (09/09/2569):** ใน `statusUpdate()` (ในทรานแซกชันเดียวกับการเขียนสถานะ+log, เฉพาะเมื่อ `$old !== $new`) — **เข้า**สถานะ `ส่ง QC รอผล` → เซ็ต `tb_planning.qc_date` (Y-m-d) + `qc_time` (H:i:s) = เวลาที่เปลี่ยนสถานะ (`now()`). **ออกจากสถานะนี้ไปสถานะอื่น = คงค่าเดิมไว้ ไม่ล้างเป็น null** (ผู้ใช้ยกเลิกการล้างค่าเมื่อ 09/09/2569 — เดิมออกแบบให้ล้าง แต่เปลี่ยนใจ). ชื่อสถานะเก็บที่ค่าคงที่ `WorkerPlanningController::QC_WAIT_STATUS` — จับคู่ด้วย**ชื่อ** เพราะ "ส่ง QC รอผล" มีหลาย id ตามแผนก (id 8/22/32/36) แต่ชื่อเหมือนกันทุกแผนก. **ไม่แตะ `qc_datetime`/`qc_status`** (คอลัมน์ที่มีอยู่แต่ผู้ใช้ขอเฉพาะ qc_date/qc_time). ค่านี้ถูกใช้ในรายงานการขาดวัตถุดิบ (คอลัมน์ วันที่/เวลาที่ส่ง QC)
- **หน้าพนักงาน QC (QC portal)** (`QcPlanningController`, `routes/qc.php` prefix `qc`, layout สลิม `layout/qc` สีม่วง ไม่มี sidebar/เมนู, 09/09/2569): ให้ emp ที่ role = **"QC"** (`Emp::QC_ROLE_NAME` + `Emp::isQc()`) ตรวจงานที่ **`tb_planning.planning_status = 'ส่ง QC รอผล'`** แล้วบันทึกผล QC. login ใช้หน้าเดิม (guard `emp`) → `AccessControl::homeUrl()` (เพิ่ม branch `isQc()`) พา QC ไป `qc.planning.index`. **route ไม่อยู่ใน `config/menu.php`** จึง pass-through `CheckAccess` (namespace `qc` ไม่ผูกเมนู) → ต้องมี middleware **`qc`** (`QcOnly`, ขึ้นทะเบียนใน Kernel) กันบัญชีอื่น. โครงเลียนแบบ Worker portal แต่ต่างที่สำคัญ:
  - **แสดง "ทุกงาน" ที่รอ QC (ไม่กรอง empno)** — QC ตรวจงานของทุกคน (ผู้ใช้เลือก 09/09/2569); ความปลอดภัยคุมด้วย middleware `qc` อย่างเดียว. `jobOrFail()` โหลดงานตาม id เฉย ๆ (ไม่มีเงื่อน empno แบบ `ownJobOrFail` ของ Worker). ค้นหารวมช่องเดียว (`red_bill_code`/`itemno`/`machine_no` LIKE) + กรอง `inplan` + ตัดงานที่ปิดแล้ว (`end_job`/`end_order`='Y', orWhereNull เหมือน Worker). เรียงตาม `qc_date`→`qc_time`→`id` (งานส่ง QC ก่อน ขึ้นก่อน)
  - **บันทึกผล = แตะเฉพาะ `qc_status` (id → `tb_qc_status`) + `qc_datetime` (`now()`) ไม่แตะ `planning_status`** ⇒ หลังบันทึก **งานยังอยู่ในตาราง** (ผู้ใช้เลือก 09/09/2569 — QC ซ้ำ/แก้ผลได้ ตารางโชว์ผล QC ล่าสุดในแถว). `resultUpdate()` validate `qc_status` `exists:tb_qc_status,id` + บล็อกฝั่ง server 2 ชั้น (งานปิดแล้ว / หลุดจากสถานะ 'ส่ง QC รอผล') ตอบ 422 + `job_closed:true` ให้ฝั่งจอปิด modal + reload. dropdown ผล QC ดึงจาก master `tb_qc_status` (`is_active='Y'`, plain `<select>` ไม่ใช่ select2). ปุ่ม "ดูรายละเอียด" เป็น modal อ่านอย่างเดียว (โชว์ผล QC + ประวัติ `tb_planning_status_log`)
  - ⚠ **ยังไม่มีบัญชี emp ที่ role=QC** (role "QC" id 7 มีในตาราง `roles` แล้ว แต่ยังไม่ผูกพนักงาน) → ต้องไปกำหนด role "QC" ให้พนักงานที่หน้า "จัดการพนักงาน" ก่อน login เข้าทดสอบได้. ⚠ ความเสี่ยงเดียวกับ Worker: ไม่มี audit trail ว่า QC คนไหนบันทึกผล (แตะแค่ 2 คอลัมน์ ไม่เขียน log)
...
- `SemiPigment` (`tb_semi_pigment`) เป็น approval workflow: `status` เป็น ENUM `request` → `approved` / `reject` (มี constants + label ภาษาไทยอยู่ใน model) เมื่อ approve แล้วจะสร้าง `result_planning` ขึ้นมา
- **กดอนุมัติ = สร้างแผนการผลิตอัตโนมัติ (01/09/2569):** เดิม `approve()` แค่เปลี่ยนสถานะเป็น `approved` แล้วผู้ใช้ต้องไปกดปุ่ม "สร้างแผน" ในหน้าอนุมัติแล้วอีกที (`convertplanning`). ตอนนี้ `SemiPigmentController::approve()` เรียก `createPlanningFromSemiPigment()` (helper ตัวเดียวกับ `convertplanning`) ต่อทันทีในทรานแซกชันเดียว → ได้ `PlanningHeader` + `Planning` + เซ็ต `result_planning_id` เลย. ครอบด้วย `DB::beginTransaction()` — ตารางเป็น InnoDB (`tb_semi_pigment`/`tb_planning`/`tb_planning_header`) จึง rollback ได้จริง: ถ้าสร้างแผนพลาดจะคืนสถานะกลับเป็น `request` ให้แก้แล้วกดใหม่ (ตอบ 500 พร้อมข้อความ). guard `if (!$sp->result_planning_id)` กันสร้างซ้ำ. **ปุ่ม "สร้างแผน" (`convertplanning`) ในหน้าอนุมัติแล้วยังคงไว้** สำหรับรายการเก่าที่อนุมัติก่อนวันนี้แต่ยังไม่มีแผน. **ฝั่ง JS ไม่ได้แก้** — ข้อความยืนยันใน `semi-pigment/index.blade.php` (`.btn_approve`) เขียนไว้ว่า "ระบบจะบันทึกข้อมูลและนำรายการนี้ไปสร้างแผนการผลิต" อยู่แล้ว และ `oTable.draw(false)` หลังสำเร็จรีเฟรชให้ป้าย "สร้างแล้ว" (badge) ขึ้นเอง
...
- **รอบการผลิต (cycles) เปลี่ยนจากตัวเลขเป็นข้อความ** (`tb_planning.cycles`, 01/09/2569): ช่อง "รอบการผลิต (Cycles)" ใน modal แก้ไข Planning item เดิมเป็นตัวเลข (`int(11)` + input `type="number"`) → เปลี่ยนเป็น**ข้อความยาวสูงสุด 25** ตามที่ผู้ใช้สั่ง. แก้ 3 จุด: (1) migration `2026_09_01_110000_change_cycles_to_varchar_on_tb_planning_table` `ALTER ... MODIFY cycles VARCHAR(25) NULL` (raw SQL เพราะไม่มี doctrine/dbal, idempotent เช็คชนิดคอลัมน์ก่อน, `down()` ถอดกลับ int ได้) — ค่าเดิม 9 แถว (1/2/3) แปลงเป็น string ครบ ไม่มีข้อมูลหาย · (2) `planning-item-form.blade.php` input `type="text" maxlength="25"` · (3) `ProductionPlanController::saveItem` validation `nullable|string|max:25` (เดิม `nullable|integer|min:0`). **คอลัมน์นี้ถูกอ้างแค่ 2 ไฟล์นี้** (blade + controller) ไม่มีใน datatable/รายงาน/export จึงไม่กระทบที่อื่น
...
- **การบรรจุ** (`tb_planning_packing`, 08/09/2569): ตารางลูกของ planning item (1 planning → **หลายแถว**) เก็บ `packing_datetime` (datetime), `weight_packing` (decimal(20,6)), `pack_remark` (text), `sort` — Model `App\Models\PlanningPacking` + relation `Planning::packings()`. **เดิมเป็นคอลัมน์เดี่ยวบน `tb_planning`** (`packing_datetie`/`weight_packing`/`pack_remark` — บรรจุได้ครั้งเดียว) → normalize เป็นตารางลูกให้เพิ่มได้หลายแถว (มิเรอร์แพทเทิร์นจาก `tb_planning_prod_method` ทั้งชุด)
  - **UI:** การ์ดสีเขียวในฟอร์มแก้ไข Planning Item (`planning-item-form.blade.php`) เป็นแถวซ้ำ + ปุ่ม "เพิ่ม" (`#btn_add_packing`) + ปุ่มลบ (`.btn_remove_packing`, เหลือแถวเดียว = ล้างค่าแทนลบ) — ช่องใช้ name array `packing_datetime[]`/`weight_packing[]`/`pack_remark[]` · ปุ่ม "จบงาน (End Job)" เติม/ล้างวันเวลาปัจจุบันที่ **แถวบรรจุแถวสุดท้าย** (เมื่อว่าง)
  - **auto-รวมน้ำหนัก → Weight Produced (09/09/2569):** พิมพ์/เพิ่ม/ลบ "น้ำหนักบรรจุได้" (`weight_packing[]`) → ช่อง **"น้ำหนักที่ผลิตได้" (`weight_produced`)** = ผลรวมทุกแถว อัปเดตสด (ฟังก์ชัน `recalcWeightProducedFromPacking()` ใน `planning-item-form.blade.php` ผูก delegated `input change blur` + เรียกท้าย handler เพิ่ม/ลบแถว). **client-side ล้วน** — `weight_produced` ยังพิมพ์เองได้ (แต่ถูกทับเมื่อแก้บรรจุ), **ไม่เรียกตอนโหลดฟอร์ม** (คงค่าเดิม), item จบงานแล้ว (ช่องบรรจุ disabled ไม่มี `name`) → ไม่ทำงาน. บันทึกผ่าน `saveItem` เดิม (ตัดคอมมาที่ loop `weight_produced`)
  - **ล็อกเมื่อจบงานแล้ว (08/09/2569):** item ที่ `end_job='Y'` (สถานะใน DB) → การ์ดบรรจุ **อ่านอย่างเดียว: เพิ่ม/แก้ไข/ลบไม่ได้** — blade เช็ค `$packing_locked = ($planning_item?->end_job ?? 'N')==='Y'` แล้วซ่อนปุ่มเพิ่ม/ลบ + render ช่องเป็น `disabled` (ไม่มี `name` → ไม่ถูกส่ง) + badge "จบงานแล้ว — แก้ไขการบรรจุไม่ได้". **ด่านฝั่ง server:** `saveItem()` คำนวณ `$was_ended` จากสถานะ **เดิม** (ก่อนบันทึก) ถ้า `='Y'` → **ข้าม `syncPackings`** (ไม่แตะแถวบรรจุ) กัน bypass ฝั่ง client และกันข้อมูลหาย. ⚠ ยึดสถานะเดิม ไม่ใช่ค่าที่ส่งมา → ครั้งที่ "เพิ่งกดปิดงาน" (เดิม N) ยังบันทึกการบรรจุได้ตามปกติ; ถ้าจะแก้บรรจุของงานที่ปิดแล้ว ต้องปลด End Job (บันทึก) แล้วเปิดใหม่
  - **บันทึก:** `ProductionPlanController::syncPackings()` (ลบทั้งหมดแล้ว insert ใหม่, ในทรานแซกชันเดียวกับ `syncProdMethods`) — ตัดคอมมาจากช่อง `weight_packing` (js-number-format) เอง · `editItem()` ส่ง `$packing_rows = $planning_item->packings`
  - **อ่านค่าเดิม (normalize เต็มรูปแบบ):** ทุกจุดที่เคยอ่านค่าเดี่ยวสรุปจากตารางลูก — `dataQuery()` มี subquery `MAX(packing_datetime) as pk_last` + `COUNT(*) as pk_count` (**ตั้ง alias คนละชื่อกับคอลัมน์เดิม** เพื่อกันชน `tb_planning.*` ตอน Yajra ห่อ derived table → โค้ดทำงานได้ทั้งก่อน/หลัง drop คอลัมน์เดิม); หน้ารายการใช้ `addColumn('packing_display')` (`orderable:false`) + Excel/PDF อ่าน `pk_last`/`pk_count` แสดงวันล่าสุด + "(N รายการ)" เมื่อ `pk_count>1`; ตัวกรองวันบรรจุใช้ `whereExists` บนตารางลูก; `OrderChangeRequestController` รวม `weight_packing` ด้วย subquery `SUM` (ใช้ `->select(...)->selectRaw(... as weight_packing)` ระบุคอลัมน์ชัด ไม่ `tb_planning.*` จึงไม่ชน)
  - **migration:** `2026_09_08_110000_create_tb_planning_packing_table` (สร้าง + คัดลอกข้อมูลเดิม 13 แถวเข้าตารางลูก, idempotent) — **รันแล้ว**. `2026_09_08_120000_drop_packing_columns_from_tb_planning` (drop 3 คอลัมน์เดิม) เป็น **cleanup ที่ทำหรือไม่ก็ได้** (โค้ดไม่พึ่งการ drop แล้ว เพราะ alias คนละชื่อ) — คอลัมน์เดิมกลายเป็น dead column (ไม่มีใครอ่าน/เขียน) ⚠ ตัว drop เป็นการลบข้อมูล (ข้อมูลถูกย้ายเข้าตารางลูกแล้ว) จะรันเมื่อสำรอง DB พร้อม
- **แปลง Order → แผนการผลิต** (`OrderController::convertplanning`, `/production-planning/order`, 10/08/2569): สร้าง `PlanningHeader` (`plan_type='ORDER'`) + `Planning` ต่อ 1 suborder — `tb_planning_header.remark` ดึงจาก `suborder.Remark` ของทุก suborder (ตัดค่าว่าง คั่นด้วย `, `, **เก็บทุกค่าแม้ซ้ำกัน**); ระวัง: ตาราง `morder` **ไม่มี** คอลัมน์ `Remark` (หมายเหตุอยู่ที่ `suborder.Remark` เท่านั้น). แต่ละ item ใส่ `remark = suborder.Remark` เหมือนเดิม
  - **red_bill_code ต้องไม่ซ้ำทุก item + เติม suffix `/Rn` เมื่อออเดอร์มีหลาย item (14/09/2569):** `tb_planning.red_bill_code` มี **unique index** (`tb_planning_red_bill_code_unique`, migration `2026_09_14_100000_add_unique_red_bill_code_to_tb_planning_table` — idempotent, **รันแล้ว** บน DB จริง). ⇒ convert จึง **stamp `orderno` เปล่าให้ทุก item ไม่ได้** (จะชนกัน): **มีหลาย suborder → `orderno/R1`, `/R2`, …** (ตาม convention เลขใบเบิกจริง เช่น `HE1576/R1..R3`) · **มี suborder เดียว → `orderno` เปล่าเหมือนเดิม**. ครอบ `DB::beginTransaction()` (tb_planning/header เป็น InnoDB) — item ใดชน unique → rollback ทั้งชุด + ตอบ 500 (เดิมไม่มี transaction). **migration ยังแก้ข้อมูลเดิมที่ซ้ำ** (2 กลุ่ม: `HI57884`, `HI57906` → `/R1`,`/R2`) + normalize `''`→NULL ก่อนสร้าง index (unique ยอม NULL ซ้ำได้ แต่ `''` ซ้ำไม่ได้)
  - **ด่านฝั่ง app ที่ `ProductionPlanController::saveItem` (14/09/2569):** normalize `red_bill_code` (`''`/เว้นวรรค → null) + เช็คซ้ำก่อนบันทึก (`Planning::where('red_bill_code',...)` ยกเว้นแถวตัวเองตอนแก้ไข) ซ้ำ → **422 + `red_bill_duplicate:true`** พร้อมข้อความไทย (จอโชว์ `response.message` ผ่าน Swal อยู่แล้ว ไม่ต้องแก้ฝั่ง client). ⚠ ช่อง Semi (`sp_red_bill_code` → `tb_semi_pigment.red_bill_code`) เป็นคนละตาราง **ไม่ได้บังคับ unique** — ถ้าต้องการต้องทำแยก
...
- **รายงานตามเครื่องจักร + จัดคิว drag & drop** (`Production\ReportController::machine/machineOptions/machineTable/machineQueueReorder/machineExcel/machinePdf`, route `production.report.machine.*`, blade `report/machine.blade.php` + partials `machine-table`/`machine-pdf`): จัดกลุ่มงาน (`tb_planning`) **ตามเครื่องจักร (`machine_no`)** → ภายในกลุ่มเรียงตาม **วัน (`day_key`)** → **คิว (`queue_sort`)** → เวลา (`job_key`) → id. ประกอบข้อมูลใน `buildMachineReport()` (ใช้ร่วมทั้งตาราง/Excel/PDF) join `tb_planning_header`+`customer`+`tb_products` (Resin/CODE/Pack/Batch/สูตร) + subquery `machine.speed_rpm` (คู่ `machine.MBX`=`machine_no`) + แนบขั้นตอน `tb_planning_prod_method` (steps). กรอง: แผนก(`dept`=company)/เครื่อง(`machine_no`)/ช่วงวันที่(`inplan`). **day_key / job_key อิง `tb_planning.inplan` (15/08/2569)**: `day_key` = วันของ `inplan`; `job_key` = วันของ inplan + เวลา `start_time` ของขั้นตอนแรก (ไม่มีเวลา → 00:00:00); งาน `inplan`=NULL → day_key/job_key = null ไปกลุ่มท้ายสุด **และลากจัดคิวไม่ได้**. **จัดคิว (drag & drop)**: SortableJS ลากทั้งงาน (`tbody.qjob`) ผ่าน handle `.qhandle`; `onMove` อนุญาตเฉพาะ **เครื่องเดียวกัน (`data-machine`) + วันเดียวกัน (`data-day`=`day_key`)** — กันข้ามเครื่อง/ข้ามวัน (การกันอยู่ **ฝั่ง client ล้วน**); `onEnd` ถ้าไม่ขยับจริง (oldIndex==newIndex) ไม่บันทึก, ไม่งั้นรวบรวม `planning_id` ในบล็อกเดียวกันตามลำดับบนจอ → POST `machineQueueReorder` เขียน `queue_sort=1..N` (เช็คแค่ ids เป็น array ไม่ว่าง + cast int; **ไม่** validate เครื่อง/วัน/สิทธิ์/มีจริง/transaction). บันทึกล้มเหลว → reload คืนลำดับจริง. เปลี่ยน `day_key` ที่ server ที่เดียว client ตามอัตโนมัติ (data-day = day_key)
  - **PDF แยก section ตามแผนก (company) — ขึ้นหน้าใหม่ต่อแผนก (02/09/2569):** เฉพาะ **PDF** (`machinePdf`) เท่านั้น — **เว็บ/Excel ไม่แตะ** (ยังเป็นตารางเดียวจัดกลุ่มตาม `machine_no` เหมือนเดิม). `buildMachineReport()` เพิ่ม 2 ฟิลด์ใน select: `tb_planning.company` + subquery `machine_group` = `` machine.`group` `` (คู่ `MBX`=`machine_no`, reserved word ต้อง backtick ใน raw SQL) — ฟิลด์เกินตัวอื่นไม่ใช้จึงไม่กระทบ. `machinePdf()` เอาบล็อกเครื่องจักร (จาก `$groups`) มา **group ตามคู่ `company` + `machine_group`** (คีย์ `company.'||'.group`) → แต่ละคู่ (แผนก+หน่วยย่อย) = 1 section: **ขึ้นหน้าใหม่** (`page-break-before` ใส่ inline เฉพาะ `!$loop->first` — **ห้ามใช้ CSS `:first-of-type`** mPDF ไม่รองรับ จะได้หน้าว่างนำ) + **พิมพ์หัวสรุปซ้ำ**. **group ต่างกัน = คนละหน้า แม้อยู่แผนกเดียวกัน** (ผู้ใช้สั่ง 02/09/2569). ภายใน section (แผนก+group เดียวกัน) เรียงบล็อกเครื่องตามลำดับเดิม (inplan เก่าสุดของบล็อก = `min`). เรียง section: แผนกตามตัวอักษร (ว่างท้ายสุด) → group แบบ `strnatcasecmp` (ว่าง/NULL = section "ไม่ระบุ" ท้ายสุดของแผนก). ไม่มีข้อมูล → คง 1 section ว่างเพื่อพิมพ์หัว + "ไม่พบข้อมูล". blade `machine-pdf.blade.php` วน `$sections` (แต่ละอันมี `dept`/`group`/`count`/`blocks`/`summary`) แทน `$groups`/`$summary` เดิม
  - **หัวสรุปต่อ section ใช้ `machineSectionSummary()` (02/09/2569):** รูปแบบ "แผนก: CP | **หน่วยย่อย: 2+VC** | ช่วงวันที่ Inplan: … | พบ N รายการ" — **เปลี่ยนป้าย "เครื่องจักร" เป็น "หน่วยย่อย"** และแสดง **`group` เดียว** ของ section นั้น (เครื่องที่ไม่มี group = "ไม่ระบุ"); `พบ N` = จำนวนเฉพาะ section (แผนก+group). **`machine.group`** = คอลัมน์ที่เพิ่ม 02/09/2569 (ดูหัวข้อ "จัดการเครื่องจักร")
  - **Excel แบ่ง section เหมือน PDF (02/09/2569):** `machineExcel` เลิกใช้หัวเดียวบนสุด (A1 title + A2 `filterSummary`) เปลี่ยนเป็นวน `machineSections()` ตัวเดียวกับ PDF — แต่ละ section (แผนก+group) พิมพ์ **หัวรายงาน + หัวสรุป `machineSectionSummary()` ("หน่วยย่อย") + ตาราง** ซ้ำ และ **ใส่ row page-break** (`setBreak('A'.($r-1), Worksheet::BREAK_ROW)` ก่อนทุก section ยกเว้นอันแรก) ให้แต่ละ section ขึ้นหน้าใหม่ตอนพิมพ์ (Xls/BIFF รองรับ). ไม่มีข้อมูล → หัว + "พบ 0 รายการ" + "ไม่พบข้อมูล"
  - **`machineSections(Collection $groups): Collection`** = helper กลางที่ทั้ง PDF/Excel เรียก — group บล็อกเครื่อง (จาก `buildMachineReport`, เป็น **Collection** ไม่ใช่ array) ตามคีย์ `company.'||'.group` แล้วเรียง (แผนก→group). ⚠ **`filterSummary()` ไม่มีผู้เรียกแล้ว** (เก็บไว้เผื่อใช้ หัวสรุปรวมทั้งรายงานแบบเดิม) — ถ้าจะแก้หัวสรุปรายงานเครื่องจักรให้แก้ที่ `machineSectionSummary()`
...
- **รายงานการขาดวัตถุดิบ** (`Production\ReportController::materialShortage/materialShortageTable/materialShortageExcel/materialShortagePdf`, route `production.report.material-shortage.*`, เมนูใต้ ProductionReport, 13/08/2569): ตารางแบนราบ (ไม่จัดกลุ่ม) ของงานใน `tb_planning` ที่ **ยังไม่ปิดงาน** (`end_job != 'Y'` **รวม NULL**) — คอลัมน์: เครื่องจักร/IN PLAN(`inplan`)/รหัส-ชื่อลูกค้า(`tb_planning_header.custno`+`customer.name`)/Order No(`tb_planning_header.orderno`)/รหัสสินค้า(`itemno`)/LOT/น้ำหนัก(`quantity`)/สถานะปัจจุบัน(`planning_status`). กรองแผนกด้วย `COALESCE(tb_planning.company, tb_planning_header.company)`, เรียง `inplan` เก่า→ใหม่ (ไม่มีวันที่ท้ายสุด). โหลดตารางผ่าน AJAX (partial `material-shortage-table`, ตารางบนเว็บ = ชุดคอลัมน์ย่อ). **Export Excel/PDF ใช้ผังคอลัมน์เต็มตามฟอร์ม Access เดิม (20 คอลัมน์)**: #, MACHINE No., IN PLAN, Revise(`senddate` กำหนดส่งทบทวน, 15/08/2569), สถานะปัจจุบัน(`planning_status`), Cust Due(`custwant` item→header), Cust no, Cust Name, SaleNo(`header.saleno`), Order Date(`header.mdate`), Order No, PRODUCT NO(`itemno`), LOT, น้ำหนัก(`quantity`, เดิมเคยแยก "น้ำ"/"หนัก" → รวมเป็นคอลัมน์เดียว 20/08/2569), ส่งชั่งสี*, เริ่มผลิต(`start_date`), วันที่ส่ง QC(`qc_date`), เวลาที่ส่ง QC(`qc_time`), สถานะ QC(`qc_status`) — *คอลัมน์ น้ำ/ส่งชั่งสี ยังไม่มีฟิลด์ใน DB จึงเว้นว่างไว้ตามผัง (Revise เดิมเว้นว่าง → ผูกกับ `tb_planning.senddate` แล้ว 15/08/2569 ทั้ง Excel คอลัมน์ F และ PDF); Excel=PhpSpreadsheet A..T, PDF=mPDF A4-L `material-shortage-pdf`. **เพิ่ม 1 คอลัมน์ "ขาด semi" ต่อจาก "สถานะปัจจุบัน" ทั้ง PDF และ Excel (15/08/2569)** — PDF พื้นเหลือง (`class="status"`) เหมือนสถานะปัจจุบัน, Excel = คอลัมน์ H (เลื่อน H..V เดิม → I..W, รวม 23 คอลัมน์). ดึงคำร้องขอ semi จาก `tb_semi_pigment` (`type='semi'`, `status` ∈ request/approved) **จับคู่ด้วย `planning_id` = `tb_planning.id`** (ไม่ใช่ itemno — เพราะ `tb_semi_pigment.itemno` = รหัสของตัว semi ที่ขาด ไม่ใช่ itemno ของงาน) + **เงื่อนไขปิดออเดอร์**: `LEFT JOIN tb_planning_header as ph` (ผ่าน `tb_semi_pigment.planning_header_id`) แล้วดึงเฉพาะ semi ที่ `ph.end_order != 'Y'` (รวม NULL) — semi ที่ปิดออเดอร์แล้วจะไม่ดึง. รวม 3 ฟิลด์จาก modal แก้ไข Semi (`itemno` ของ semi + `semi_code` + `primary_color`) ไว้ใน**คอลัมน์เดียว** คั่นด้วย ", " (ตัดค่าว่าง/ซ้ำ; งานที่ไม่มีคำร้องขอ semi = เว้นว่าง). assemble ใน `buildMaterialShortageReport()` → attach `lack_semi` ต่อแถว. **เพิ่ม 1 คอลัมน์ "ขาดวัตถุดิบ" ต่อจาก "ขาด semi" ทั้ง PDF และ Excel (20/08/2569; เปลี่ยนที่มา + เปลี่ยนชื่อหัวคอลัมน์ 25/08/2569)** — PDF พื้นเหลือง (`class="status"`), Excel = คอลัมน์ I (รวม 23 คอลัมน์ A..W หลังยุบ "น้ำ"/"หนัก" เป็น "น้ำหนัก" 20/08/2569). **เดิม (20/08/2569) หัวคอลัมน์ชื่อ "ขาด Pigment" และดึง `itemno` ของคำขอ pigment จากตาราง `tb_pigment` — ยกเลิกแล้ว**. ตอนนี้แสดง **ข้อความอิสระจาก `tb_planning.shortage_remark`** ของงานแถวนั้นตรง ๆ (ค่าที่ผู้ใช้พิมพ์เองในช่อง "ขาดวัตถุดิบ (Shortage Remark)" ของฟอร์มแก้ไข planning item — `planning-item-form.blade.php`, บันทึกผ่าน `ProductionPlanController::saveItem`): `buildMaterialShortageReport()` เพิ่ม `tb_planning.shortage_remark` ใน select แล้ว `$row->lack_pigment = trim((string) $row->shortage_remark)` (ชื่อตัวแปร `lack_pigment` คงเดิมเพื่อไม่ต้องแก้ blade/Excel ที่อ่านค่า). **คิวรี `tb_pigment` (Model `Pigment`) ในเมธอดนี้ถูกลบออกแล้ว** พร้อม import `App\Models\Pigment` (ฟีเจอร์ pigment เดิม/ตาราง `tb_pigment` ไม่ถูกแตะ ยังใช้ที่ `PigmentController`). **หมายเหตุ:** ยังไม่มีตารางสต๊อกวัตถุดิบผูก planning ใน DB → รายงานนี้ = "งานค้าง (ยังไม่ปิดงาน)"; หากต้องเช็คปริมาณวัตถุดิบจริงภายหลัง ให้เสริมเงื่อนไขใน `buildMaterialShortageReport()`
  - **ซ่อนเพิ่ม Cust no + Order Date + ย่อ Cust Name ทั้ง 3 ที่ (เว็บ/PDF/Excel) — 10/09/2569:** ตามที่ผู้ใช้สั่ง — คอมเมนต์ปิดคอลัมน์ **Cust no** และ **Order Date** (สถานะปัจจุบันถูกซ่อนอยู่แล้วตั้งแต่ 03/09/2569 ใน 3 ที่นี้) + ทำ **Cust Name ให้แคบลง + ตัด `…` เมื่อยาวเกิน**
    - **ตารางเว็บ** (`material-shortage-table.blade.php`): คอมเมนต์ th+td ของ Cust no/Order Date/สถานะปัจจุบัน. Cust Name ครอบด้วย `<span class="d-inline-block text-truncate" style="max-width:200px" title="ชื่อเต็ม">` — `text-truncate` (Bootstrap: overflow hidden + ellipsis + nowrap) ทำ `…` ให้เอง (ตารางเป็น `text-nowrap` อยู่แล้ว) + `title` โชว์ชื่อเต็มตอน hover. ความกว้าง 200px = ประมาณช่อง ขาด semi (`col-lack` ไม่ล็อกกว้าง ขยายตามเนื้อหา)
  - **ตารางเว็บเปลี่ยนเป็น DataTables ฝั่ง client (paging/search/sort/info/length) — 10/09/2569:** เดิม partial `material-shortage-table` render ทุกแถวเป็นตารางเปล่า ๆ (AJAX โหลด HTML เข้า `#reportResult`) → ตอนนี้ init `$('#materialShortageTable').DataTable()` **แบบ client-side** (ไม่ใช่ serverSide Yajra) เพราะรายงานนี้**ประกอบข้อมูลใน PHP** (attach `lack_semi`/`lack_pigment` + filter เฉพาะงานที่ขาดจริง ใน `buildMaterialShortageReport`) จึงไม่มี query builder ให้ Yajra จัดหน้า/เรียงที่ SQL — มิเรอร์แพทเทิร์นเดียวกับหน้า **order-change-request** (client-side DataTables). จุดที่แก้:
    - **`material-shortage-table.blade.php`**: `@forelse/@empty` → `@foreach` (ตัด empty-row colspan ออก ให้ DataTables โชว์ "No data available" เอง) · เพิ่ม `data-order` ให้คอลัมน์ **Revise/Cust Due** (`Y-m-d`) และ **น้ำหนัก** (ตัวเลขดิบ) เพื่อเรียงตามค่าจริง ไม่ใช่ข้อความ `d/m/Y`/มีคอมมา · Cust Name ใส่ `data-order="{{ ชื่อเต็ม }}"` (เรียงด้วยชื่อเต็ม ไม่ใช่ข้อความที่ถูกตัด `…`) · เพิ่ม `w-100` ที่ `<table>` · div "พบ N รายการ" คงไว้เหนือตาราง
    - **`material-shortage.blade.php`**: ถอด CSS ตรึงหัวตาราง sticky + `#reportResult{max-height/overflow}` ออก (DataTables จัดหน้าแทน) — **คง** `.col-lack`/`.col-custdue` highlight · JS: เก็บ instance `msTable`, ใน `loadReport()` **destroy ตัวเก่าก่อนแทนที่ HTML** แล้ว `initMaterialShortageTable()` ใน success · คอลัมน์ `#` (target 0) `orderable:false` + **renumber 1..N ข้ามหน้า** ด้วย recipe `msTable.on('order.dt search.dt', … column(0,{search:'applied',order:'applied'}).nodes()…)` · `order: []` = คงลำดับเริ่มต้นจาก server (สถานะ → เครื่อง → inplan)
    - **ตัวกรองแผนก (`#searchDept`) ยังเป็น server filter** (กด "ค้นหา" → reload partial + re-init DataTable) ไม่ได้ย้ายเข้า DataTables — เพราะ dept ต้องส่งไป export links (Excel/PDF) ด้วย. DataTables คุมแค่ paging/search/sort ภายในชุดที่โหลดมา
    - ⚠ **ไม่ตั้ง `language` (ใช้ default อังกฤษ) ให้ตรงกับหน้าอื่นในระบบ** (serverSide Yajra ที่อื่นก็ default อังกฤษ). DataTables Bootstrap5 โหลดที่ `layout/inc_header` + `inc_js` อยู่แล้วทั้งระบบ
    - **Excel** (`materialShortageExcel`): renumber คอลัมน์ให้ต่อเนื่อง `L → J` (ตัด Cust no/Order Date ออก คอลัมน์เลื่อน) — คู่ `$headers`/`$cols` เหลือ 10 ช่อง (A..J: #, แผนก, เลขที่ใบแดง, Revise, ขาดวัตถุดิบ, ขาด semi, Cust Due, **Cust Name(H)**, PRODUCT NO(I), น้ำหนัก(J)). Cust Due พื้นแดงยังอยู่คอลัมน์ G เหมือนเดิม. Cust Name (H) เลิก `setAutoSize(true)` → `setWidth(22)` + `setWrapText` (เดิม autosize ยืดตามชื่อยาว). setCellValue ที่ปิดมีคอมเมนต์กำกับวันที่ + วิธีเปิดคืน
    - **PDF** (`material-shortage-pdf.blade.php`): คอมเมนต์ th+td Cust no/Order Date (colspan แถวว่าง `12 → 10`). 🔴 **mPDF 8.3.1 ไม่รองรับ `text-overflow: ellipsis` ใน table cell — มันจะ wrap ขึ้นบรรทัดใหม่แทน** (ทดสอบ render จริงยืนยันแล้ว) ⇒ **ตัดข้อความ + เติม `…` ฝั่ง PHP** ด้วย `\Illuminate\Support\Str::limit($it->cust_name, 40, '…')` (ไทยตัวแคบ 40 ตัวพอดี 1 บรรทัด; อังกฤษยาวมากอาจ wrap 2 บรรทัด พบน้อย). **width ของทุก th ที่แสดงอยู่ตั้งให้รวม = 100%** (`# 3 / แผนก 5 / เลขที่ใบแดง 9 / Revise 7 / ขาดวัตถุดิบ 16 / ขาด semi 17 / Cust Due 7 / Cust Name 17 / PRODUCT NO 11 / น้ำหนัก 8`) — **สำคัญ:** ถ้ารวมไม่ครบ 100% mPDF จะกระจายส่วนที่เหลือมั่ว ทำให้ Cust Name กว้างเกินแล้วเหลือช่องว่างโหว่ (bug ที่เจอตอนแรก). Cust Name = 17% = เท่าช่อง ขาด semi ตามที่ผู้ใช้ขอ
...
**การเดินเลขที่ใบสั่ง** (`allocateOrderno`, 18/08/2569): ค่าใน `orderrun` คือ **"เลขล่าสุดที่ใช้ไปแล้ว"** (ยืนยันจาก `orderrun.w = 24564` = ใบ `WI24564` ที่มีอยู่จริง) → เลขถัดไป = ค่านั้น **+1** แล้ว **ข้ามเลขที่มีใบสั่งอยู่จริง** เผื่อค่าใน `orderrun` ไม่ตรงกับข้อมูล (ต้องเช็คทุกประเภทที่ใช้คอลัมน์เลขรันร่วมกัน เช่น CM/CI ใช้ `c` ด้วยกัน). **ปุ่ม "เพิ่มใบสั่งซื้อใหม่" ในฟอร์ม = สร้างใบสั่งซื้อทันที** (12/09/2569 — แก้ปัญหาเลขที่ออเดอร์ชนกัน): `orderNew()` → `POST order/create` (`OrderController::create`) → ออกเลขด้วย `allocateOrderno()` แล้ว insert `morder` (`Orderno` / `Mdate`=now / `Emp`=ผู้ล็อกอิน) ทันที → ฝั่งจอ `orderOpen(orderno)` เปิดใบนั้นในโหมด `edit` แบบเดียวกับตอนกดบันทึกสำเร็จ ⇒ กดบันทึกต่อ = `save()` โหมด `update`. **เดิม** ปุ่มนี้เรียก `nextOrderno` ที่แค่อ่านเลขมาโชว์ แล้วค่อยจองเลขตอนกดบันทึก ⇒ หลายคนเปิดฟอร์มพร้อมกันเห็นเลขเดียวกัน (`nextOrderno` ยังอยู่แต่ฟอร์มเลิกเรียกแล้ว). ⚠ ผลที่ตามมา: กดปุ่มแล้วปิดฟอร์มโดยไม่บันทึก = ใบเปล่า (ไม่มีลูกค้า/รายการ) ค้างอยู่ใน `morder` และ `appv` ว่างจึง**โผล่ในคิวอนุมัติใบสั่งซื้อ**ด้วย · `lockForUpdate()` ใน `allocateOrderno()` ไม่มีผลกับ MyISAM (ดูหัวข้อ MyISAM)

**คอลัมน์ "สถานะ" ในตารางรายการ + ตัวกรองสถานะ** (19/09/2569 ตามที่ผู้ใช้สั่ง) — สถานะ **การอนุมัติใบสั่งซื้อ** 3 แบบ ใช้กติกาเดียวกับ `OrderApprovalController::approvableQuery()`

| สถานะ (key) | เงื่อนไข | badge | ข้อมูลจริง |
|---|---|---|---|
| `approved` อนุมัติแล้ว | `appv` ไม่ใช่ NULL/0 | เขียว (+ วันที่จาก `appvDT` บรรทัดล่าง) | 3,470 ใบ |
| `waiting` รออนุมัติ | `appv` เป็น NULL หรือ 0 | เหลือง | 11 ใบ |
| `reserve` ใบจอง | ตัวอักษรที่ 2 ของ `Orderno` = `R` (CR/HR/WR) — **ไม่ต้องอนุมัติ** | เทา | 2 ใบ |

- 🟢 **นิพจน์ SQL อยู่ที่เดียว: `OrderController::APPV_STATUS_SQL`** (`CASE … END`) — ใช้ทั้ง **คอลัมน์ที่แสดง** (`->selectRaw(... as appv_status)` ใน `datatable()`) และ **ตัวกรอง** (`->whereRaw(... = ?)` ใน `applyFilters()`) ⇒ สิ่งที่กรองได้ตรงกับ badge ที่เห็นเสมอ **แก้กติกาต้องแก้ที่นี่ที่เดียว**
- ป้าย/สี badge อยู่ที่ **`OrderController::APPV_STATUSES`** (public const) — ส่งให้ blade เป็น `$appv_statuses` ทั้งจาก `index()` (dropdown ตัวกรอง) และ `datatable()` (badge ในตาราง) · **อย่า hard-code ป้ายซ้ำใน blade**
- ตัวกรองรับเฉพาะ key ที่มีใน `APPV_STATUSES` (`isset()`) — ค่าแปลก ๆ ถูกเมิน ไม่กรอง และไม่หลุดเข้า SQL
- ช่องกรองใช้ class `p_search` จึงเข้าระบบตัวกรองกลางเอง (ส่งเข้า query · นับจำนวนบนปุ่มล้างตัวกรอง · ปุ่มล้าง) + เพิ่มป้าย `appv_status` ใน `FILTER_LABELS` ให้ขึ้นในแถบสรุป
- ⚠ **คอลัมน์นี้ยัง sort ไม่ได้** — ไม่ได้ใส่ใน whitelist `$sortable` ของ `datatable()` (ถ้าจะเปิด: เพิ่ม `'appv_status' => 'appv_status'` แล้วใส่ `th-sort` ที่หัวคอลัมน์)
- ⚠ แถวว่าง (`@empty`) ใช้ `colspan="10"` แล้ว — **เพิ่มคอลัมน์อีกต้องขยับตาม**

**ทุกการเขียนข้อมูลในหน้า `/order` ต้องโหลดตารางรายการใหม่** (19/09/2569 ตามที่ผู้ใช้สั่ง) — เรียก **`loadData(page)`** (ยิง `GET order/datatable`) ในขา `done` ของทุก endpoint ที่เขียนข้อมูล ไม่ใช่แค่ตอนบันทึกใบสั่งซื้อ

| การกระทำ | endpoint | เรียกที่ |
|---|---|---|
| เพิ่มใบสั่งซื้อใหม่ | `POST order/create` | `orderNew()` |
| บันทึกใบสั่งซื้อ | `POST order/save` | `saveOrder()` |
| **อนุมัติ / ยกเลิกอนุมัติ** | `POST order/order-approval/approve` | **`oaAfterApprove()`** |
| **บันทึกใบขออนุมัติราคา** | `POST order/price-approval/save` | **`saveApproval()`** |
| **ลบใบขออนุมัติราคา** | `POST order/price-approval/delete` | **`approvalDelete()`** |
| **เพิ่มสถานที่ส่ง** | `POST order/dvpoint` | **`addDvpointPrompt()`** (เฉพาะตอนเพิ่มจริง ไม่ใช่ค่าที่มีอยู่แล้ว) |

- 🔴 **ตัวที่สำคัญที่สุดคือขาอนุมัติ** — ตั้งแต่มีคอลัมน์ "สถานะ" (19/09/2569) การอนุมัติทำให้ค่าในตารางเปลี่ยน ถ้าไม่โหลดใหม่ badge จะค้างเป็น "รออนุมัติ" ทั้งที่อนุมัติไปแล้ว
- `loadData()` เรียก `showTableLoading()` อยู่แล้ว จึงมี spinner ระหว่างโหลด (ดูหัวข้อ `showTableLoading()` ใน "ข้อตกลงและข้อควรระวัง")
- ⚠ **ไม่ต้องเรียกกับ `POST order/price-approval/unlock` และ `/lock`** — 2 ตัวนั้นเป็นการปลด/ล็อกโหมดอนุมัติใน session ไม่ได้เขียนข้อมูลธุรกิจ

**ตัวกรองหน้ารายการ** (`OrderController::applyFilters`): ช่องค้นหารวม (`search`) ค้น LIKE ข้าม `morder.Orderno` / `PO` / `Custno` / `Custname` / `customer.name` / **รหัสสินค้าในใบ** (`whereExists` บน `suborder.Itemno`) · **ช่องค้นหา "รหัสสินค้า" แยกของตัวเอง (`input[name="itemno"]` ในแถวตัวกรองที่ 2 → param `itemno`, 16/09/2569)** — `whereExists` บน `suborder.Itemno` LIKE เหมือนกัน แต่แยกช่องเพื่อกรองเจาะจง (รวมกับตัวกรองอื่นเป็น AND) · ⚠ **ช่องรวมยังค้นรหัสสินค้าได้เหมือนเดิม** — ตอนแรกถอดออกจากช่องรวมแล้วผู้ใช้สั่งให้คืน (16/09/2569) ⇒ ช่องแยกเป็นช่อง *เพิ่ม* ไม่ใช่การย้าย · ช่องอื่น: ประเภทใบสั่ง (`order_type`) · ผลิตที่ (`company`) · ช่วงวันที่สั่ง (`date_from`/`date_to`) · ช่องใหม่ใช้ class `p_search` จึงเข้าระบบตัวกรองกลางเอง (ส่งเข้า query · นับจำนวนบนปุ่มล้างตัวกรอง · ปุ่มล้าง) เหลือแค่เพิ่มป้ายใน `FILTER_LABELS` ให้ขึ้นในแถบสรุป

**แถบประเภทใบสั่งเป็นปุ่มกลุ่ม (segmented) — 21/09/2569 ตามที่ผู้ใช้สั่ง** — เดิมเป็น radio วงกลม 12 ตัวในกล่องพื้นม่วง ตอนนี้เป็นปุ่มติดกันเป็นก้อน **3 ชุดตามตัวอักษรแรก C / H / W** (ชุดละ 4 ปุ่ม เรียง M → I → E → R) บนการ์ดขาว + ป้าย "ประเภทใบสั่ง" หน้าแถบ
- 🔴 **`<input>` ยังเป็น radio `name="order_type_form"` `id="o_type_XX"` + `onchange="onOrderTypeChange()"` เหมือนเดิมทุกตัว** — แค่ซ่อนด้วย `.btn-check` แล้วให้ `<label class="of-tbtn">` เป็นตัวปุ่ม ⇒ JS เดิมทุกตัวทำงานต่อได้ (`orderNew` · `fillOrderForm` · `setOrderFormMode` ที่ `.not('[name="order_type_form"]')`) **อย่าเปลี่ยนเป็น `<button>`**
- การแบ่ง 3 กลุ่มคำนวณใน blade (`$type_groups` จาก `$type_rows`) — **ไม่ได้แก้ `OrderController::TYPE_ROWS`** (ยังเป็น 2 แถว × 6 เหมือนเดิม)
- CSS อยู่ใน `<style>` ของ `order/index.blade.php` บล็อก "แถบประเภทใบสั่ง — ปุ่มกลุ่ม" · class เดิม `.of-typerow` / `.of-typepair` ถูกแทนด้วย `.of-typeset` / `.of-tbtn` · `.of-btn-new` **คงชื่อเดิมไว้** เพราะ JS ใช้เป็น selector (ปิดปุ่มระหว่างยิง request + ยกเว้นไม่ให้ disable ตอนโหมด idle)
...
- `new` — ~~กดปุ่ม "เพิ่มใบสั่งซื้อใหม่" ในฟอร์ม (`orderNew`) → ปลดล็อกทุกช่อง + ใส่เลขที่ที่คาดว่าจะได้~~ **ฟอร์มไม่เข้าโหมดนี้แล้ว** (12/09/2569) — ปุ่มนั้นสร้างใบสั่งซื้อทันทีแล้วเปิดเป็นโหมด `edit` (ดูหัวข้อการเดินเลขที่ใบสั่ง)
...
**กติกาการบันทึก:**
- 🔒 **ใบที่อนุมัติแล้ว = แก้ "ราคาขาย" (`morder.price`) ไม่ได้ (21/09/2569 ตามที่ผู้ใช้สั่ง)** — เกณฑ์ "อนุมัติแล้ว" ใช้ `self::checked($order->appv)` ตัวเดียวกับทั้งไฟล์ (`appv` ไม่ใช่ NULL/0) · ช่องอื่นในใบยังแก้ได้ตามปกติ
  - **ด่านฝั่ง server:** `headerPayload()` รับพารามิเตอร์ **`$approved`** เพิ่ม (ตัวที่ 5) → **`unset($data['price'])` เมื่อ `update` + อนุมัติแล้ว** ⇒ ยิง POST ตรงมาก็แก้ราคาไม่ได้ · `save()` ขา update อ่าน **`appv` จริงจาก DB** (`->first(['appv'])` ที่เปลี่ยนมาจาก `->exists()` เดิม) แล้วส่งเข้าไป — **ไม่เชื่อค่าที่ฟอร์มส่งมา**
  - **ฝั่งจอ:** `setOrderPriceLocked(locked)` ใน `order/index.blade.php` ตั้ง `#o_price` เป็น **`readonly`** (ไม่ใช่ `disabled` — ค่ายังถูกส่งไปกับ `saveOrder()` ตามปกติ และไม่ชนกับ `setOrderFormMode()` ที่คุม `disabled` ทั้งฟอร์ม) + `bg-light` + badge เขียว **"อนุมัติแล้ว"** (`#o_price_locked` ข้าง label ใน `order/form.blade.php`) · เรียกจาก `fillOrderForm()` (`!!o.appv`) และ `clearOrderForm()` (ปลดล็อก)
  - `orderPayload()` ส่ง **`appv`** (bool ผ่าน `checked()`) กลับมาให้ฝั่งจอด้วย — เดิมไม่ได้ส่ง

**badge สถานะบนหัว modal ใบสั่งซื้อ** (21/09/2569 ตามที่ผู้ใช้สั่ง) — บอกสถานะอนุมัติของใบที่เปิดอยู่ (`#o_appv_status` ใน `order/index.blade.php`) · **ไม่มีป้ายกำกับคำว่า "สถานะ"** — ตัวป้ายใน badge บอกความหมายอยู่แล้ว · วันเวลาอนุมัติอยู่**ใน badge เดียวกัน** ("✓ อนุมัติแล้ว เมื่อ 21/09/26 13:58")
- **วางไว้ "นอก" `.modal-title` โดยตั้งใจ** (บนพื้นขาวของ `modal-header` ถัดจากแถบ teal) **ชิดขวา**
- **สี + ไอคอน มาจาก `APPV_STATUSES` ทั้งคู่ ไม่ hard-code ใน JS**
  - **สีเข้มกว่าตารางรายการ** — `fillOrderStatus()` แปลงโทนอ่อน `bg-label-*` เป็นทึบ `bg-*` ด้วย `.replace()` เฉพาะ badge ตัวนี้ (ตารางยังใช้โทนอ่อนเหมือนเดิม) · ⚠ `bg-warning` ของ theme เป็น **ส้ม `#FF9F43`** ตัวอักษรขาวอ่านยาก จึงเติม **`text-dark`** ให้ตัวนั้นตัวเดียว
  - **`icon` เป็น key ที่เพิ่มเข้า `APPV_STATUSES`** (`ti-circle-check` / `ti-clock` / `ti-bookmark`) — ⚠ เดิมใส่ `ti-circle-check` ให้ทุกสถานะใน JS ทำให้ **"รออนุมัติ" ขึ้นเครื่องหมายถูก** ความหมายขัดกัน (ผู้ใช้ทัก 21/09/2569) · คอลัมน์สถานะในตารางไม่ได้ใช้ key นี้ จึงไม่กระทบ
- 🔴 **2 อย่างที่เป็น `position: absolute` จึงไม่กินที่ในแนว flex — ต้องเว้นระยะเองทั้งคู่** (CSS `#orderModal .modal-header #o_appv_status`)
  - **สามเหลี่ยมเฉียง `::after` ของ `.modal-title`** ยื่นออกขวา 65px — ถ้า badge ชิดซ้ายจะไปทับ (ผู้ใช้ทัก) · ตอนนี้ใช้ **`margin-left: auto`** ดันไปขวาสุด จึงพ้นไปเอง (เคยใช้ `margin-left: 77px` ตอนที่ยังชิดซ้าย)
  - **ปุ่มปิด** ที่ theme ตั้ง `position: absolute; right: 1rem` — ต้อง **`margin-right: 2.5rem`** ไม่งั้น badge มุดไปอยู่ใต้ปุ่ม
- ⚠ **เคยย้ายไปมาหลายตำแหน่งก่อนลงตัวที่นี่** — แถบเขียวเต็มความกว้างบนสุดของฟอร์ม · แถวแรกในการ์ด "ข้อมูลใบสั่งซื้อ" · บรรทัดแรกของ `modal-body` พร้อมป้าย "สถานะ" · มุมขวาของแถบ `.of-typebar` **ถูกปฏิเสธทั้งหมด** — ถ้าจะย้ายอีก ให้ถามผู้ใช้ก่อน
- 🟢 **ใช้กติกา/ป้าย/สี ชุดเดียวกับคอลัมน์ "สถานะ" ในตารางรายการ** — ไม่มีการเขียนเงื่อนไขซ้ำที่ไหนเลย
  - **สถานะคิดที่ SQL** ผ่าน **`OrderController::appvStatusOf($orderno)`** ซึ่งเรียก **`APPV_STATUS_SQL`** ตัวเดิม (⚠ **ห้ามคิดสถานะใหม่ใน PHP** ไม่งั้นฟอร์มกับตารางจะไม่ตรงกัน) → `orderPayload()` ส่งกลับเป็น **`appv_status`** (+ `appvDT`)
  - **ป้าย/สี** มาจาก `APPV_STATUSES` ที่ `index()` ส่งเป็น `$appv_statuses` อยู่แล้ว → blade ฝังเป็นตัวแปร JS **`APPV_STATUSES`** ให้ฝั่งจออ่าน
- **`fillOrderStatus(status, appvDT)`** ใน `order/index.blade.php` เติม badge + วันเวลา · เรียกจาก `fillOrderForm()` และ `clearOrderForm()` (ล้างเป็น `—` กันสถานะค้างจากใบก่อนหน้า)
  - ⚠ ตั้งคลาสด้วย **`.attr('class', ...)` ไม่ใช่ `.addClass()`** — ไม่งั้นคลาสสีจะสะสมทับกันเมื่อเปิดใบที่สถานะต่างกันติด ๆ กัน
  - วันเวลาอนุมัติ (`morder.appvDT`) โชว์**เฉพาะสถานะ `approved` และมีค่าจริง**
- **ใบจอง (ตัวที่ 2 ของเลขที่ใบ = `R`) มาก่อนเสมอ** — ใบจองที่ `appv` ถูกติ๊กก็ยังขึ้น "ใบจอง" ไม่ใช่ "อนุมัติแล้ว" (ตรงกับตารางรายการ เพราะเป็นนิพจน์เดียวกัน) · ทดสอบครบ 5 เคสแล้ว 21/09/2569
- ⚠ เคยลองวาง badge นี้ไว้**บนหัว modal** และเป็น**แถบเขียวเต็มความกว้าง** — **ถอดออกทั้งคู่แล้ว** (21/09/2569) ผู้ใช้ต้องการให้อยู่ในตัวฟอร์ม
  - **ยกเลิกอนุมัติแล้วแก้ราคาได้อีกครั้ง** (ยึดสถานะ ณ ตอนกดบันทึก) — แต่ UI ยังเปิดใบที่อนุมัติแล้วในฟอร์มอนุมัติไม่ได้ (ดูหัวข้อ morderAPPV)
  - ⚠ **ยังไม่ได้กัน: `oaAfterApprove()` ไม่ได้สั่งล็อกช่องราคาให้ modal ใบสั่งซื้อที่เปิดค้างอยู่** — ถ้าอนุมัติขณะเปิดใบนั้นค้างไว้ ช่องจะยังพิมพ์ได้จนกว่าจะเปิดใบใหม่ (ด่าน server ยังกันการเขียนอยู่)
  - 🟢 **ใบที่อนุมัติแล้ว "ข้ามด่านราคา" (21/09/2569 ตามที่ผู้ใช้สั่ง)** — `save()` อ่าน `appv` จาก DB ไว้ในตัวแปร `$approved` **ก่อน**เรียก `checkPriceFloor()` แล้วเรียกด่านเฉพาะเมื่อ `!$approved` (ตัวแปรเดียวกันถูกส่งต่อเข้า `headerPayload()` ⇒ query ครั้งเดียว)
    - **เหตุผล:** ราคาถูกล็อกไม่ให้แก้อยู่แล้ว การตรวจราคาที่ส่งมาจึงไม่มีผลกับค่าที่จะเขียน — ถ้าไม่ข้าม ใบเก่าที่ราคาต่ำกว่าเกณฑ์ปัจจุบันจะ **แก้ช่องอื่นแล้วบันทึกไม่ได้เลย** (วัดจากใบที่อนุมัติแล้ว 300 ใบล่าสุด: **โดนบล็อก 101 ใบ = 33.7%**)
    - ⚠ **ใบที่ยังไม่อนุมัติยังโดนด่านเต็ม ๆ เหมือนเดิม** และ **ยกเลิกอนุมัติแล้วด่านกลับมาทำงาน** (ทดสอบครบ 3 สถานะ 21/09/2569)
- 🔒 **ช่อง "วันที่" (`morder.Mdate`) แก้ไม่ได้แล้ว (19/09/2569 ตามที่ผู้ใช้สั่ง)** — เดิม (18/08/2569) ผู้ใช้แก้ได้และบันทึกค่าที่แก้ทั้ง insert/update. ตอนนี้ **server เป็นคนกำหนดค่าเดียว** ตอนกดปุ่ม "เพิ่มใบสั่งซื้อใหม่" (`create()` → `Mdate => now()`) แล้วห้ามแก้อีก
  - **ฝั่งจอ 2 ชั้น:** `#o_Mdate` เป็น `readonly` (`order/form.blade.php` — ถอด `placeholder` บอกรูปแบบออกด้วย เพราะไม่ได้พิมพ์แล้ว) + init flatpickr ของ `.flatpickr-datetime` ตั้ง **`clickOpens: false`** และ `allowInput: false` (`order/index.blade.php`) ⇒ คลิกแล้วปฏิทินไม่เด้ง · ⚠ `readonly` อย่างเดียวไม่พอ ปฏิทินยังเด้งและเซ็ตค่าให้ได้
  - **ยังต้อง init flatpickr อยู่** เพราะ `setFpDateTime()` ใช้ instance นี้จัดรูปแบบค่าที่โหลดมาจากใบเดิม (และ `clearOrderForm()` ตั้งวันเวลาปัจจุบัน) — การตั้งค่าด้วยโปรแกรมยังทำได้ตามปกติ
  - **ด่านฝั่ง server:** `headerPayload()` รับพารามิเตอร์ `$mode` เพิ่ม → **`unset($data['Mdate'])` เมื่อ `$mode === 'update'`** ⇒ ยิง POST ตรงมาก็แก้วันที่ไม่ได้ · **ขา `insert` ยังเขียน `Mdate` เหมือนเดิม** (ไม่งั้นคอลัมน์ว่าง) — `save()` ส่ง `'insert'` / `'update'` ให้ตรงกับ mode
  - ฟอร์มยังส่งค่า `Mdate` มาใน payload เหมือนเดิม (ไม่ได้แก้ `saveOrder()`) แต่ฝั่ง update ทิ้งค่านั้น
  - **เลขที่ใบสั่ง (`Orderno`) ปลอดภัยอยู่แล้ว** ไม่ต้องแก้ — `headerPayload()` ไม่เคยมี key นี้ ⇒ ไม่เขียนทับ และช่องเป็น `readonly` ทุกโหมดยกเว้น `idle` (ฟอร์มเปล่า ยังไม่มีใบให้แก้ — ใช้พิมพ์เลขใบเดิม + Enter เพื่อเปิดใบนั้น ผู้ใช้ยืนยันให้คงไว้ 19/09/2569)
  - ⚠ **ยังไม่ได้กัน:** ขา `update` ใช้ `Orderno` ที่ส่งมาจากฟอร์มเป็นตัวชี้แถว (`where('Orderno', ...)`) ⇒ แก้ค่าใน devtools แล้วยิงบันทึก = ไปทับหัวใบ**ใบอื่น**ได้ (ยังไม่มีการตรวจว่าตรงกับใบที่เปิดอยู่)
...
### เพิ่ม "สถานที่ส่ง" ได้จากฟอร์มใบสั่งซื้อ — 19/09/2569

เดิมช่อง **สถานที่ส่ง** (`#o_DVpoint`) เลือกได้เฉพาะค่าที่มีอยู่แล้วใน `naddress` ของลูกค้ารายนั้น — ถ้าไม่มีต้องออกไปเพิ่มที่เมนู "ฐานข้อมูลลูกค้า" ก่อน ตอนนี้มีปุ่ม **"+ เพิ่ม"** ข้าง label เพิ่มได้ในฟอร์มเลย

- **endpoint:** `POST order/dvpoint` → `OrderController::addDvpoint()` → เขียนลง **`naddress`** (`Custno` + `DVpoint`)
- 🟢 **เขียนลงตาราง master ตัวเดียวกับเมนู "ฐานข้อมูลลูกค้า"** ⇒ ค่าที่เพิ่มจากตรงนี้จะไปโผล่ในฟอร์มลูกค้าด้วย (ตั้งใจ — เป็นสถานที่ส่ง *ประจำ* ของลูกค้า ไม่ใช่ค่าเฉพาะใบ)
- **ตรวจฝั่ง server 4 ชั้น** (422 ทุกกรณี): ไม่ระบุลูกค้า · รหัสลูกค้าไม่มีใน `customer` · ไม่กรอกสถานที่ส่ง · **ยาวเกินความยาวคอลัมน์**
  - ⚠ `naddress.DVpoint` = **varchar(20)** — อ่านความยาวจริงจาก schema (ไม่ hardcode) แล้ว **ตีกลับให้ผู้ใช้แก้เอง ไม่ตัดเงียบ ๆ** ต่างจาก `clampToColumns()` ที่ใช้ตอนบันทึกใบสั่งซื้อ (อันนั้นเป็นค่าที่ระบบเติมให้ ส่วนอันนี้ผู้ใช้พิมพ์เองและกำลังสร้างค่าใหม่ในตาราง master)
- **เพิ่มซ้ำ = ไม่ error** — ตอบ 200 พร้อมธง `existed: true` แล้วให้ฝั่งจอเลือกตัวนั้นให้เลย (PK ซ้ำไม่ได้อยู่แล้ว)
- response แนบ `dvpoints` (รายการใหม่ทั้งชุด) + `dvpoint` (ค่าที่เพิ่ง เพิ่ม/เจอ) → ฝั่งจอเรียก **`fillDvpoints()` ตัวเดิม** เติมรายการ + เลือกให้ (ไม่ได้เขียนโค้ดวาด option ซ้ำ)
- ⚠ **ปุ่มวางข้าง label ไม่ใช่ใน `input-group`** — `enhanceSelects()` แปลง `<select>` เป็น select2/bootstrap-select ซึ่งห่อ element ใหม่ ทำให้ `input-group` เพี้ยน
- ⚠ **Swal ที่มีช่องกรอกต้องตั้ง `target: #orderModal` + `heightAuto: false`** ไม่งั้น focus trap ของ Bootstrap modal ดึงโฟกัสกลับ พิมพ์ไม่ได้ (แพทเทิร์นเดียวกับ Swal ในฟอร์ม MK / ฟอร์มอนุมัติใบสั่งซื้อ)
- ปุ่มถูก `disabled` เองตอนฟอร์มอยู่โหมด `idle` (ผ่าน `setOrderFormMode()` ที่ล็อกทุก `button` ใน modal) — เพิ่มได้เฉพาะตอนเปิดใบแล้ว ซึ่งถูกต้องเพราะต้องรู้ลูกค้าก่อน
- ⚠ **ความเสี่ยงเดิม:** route นี้ป้องกันแค่ `auth` + สิทธิ์ `order read` — **ไม่มีสิทธิ์แยกสำหรับการเขียนข้อมูลลูกค้า** ⇒ ใครเข้าเมนู Order ได้ ก็เพิ่มสถานที่ส่งให้ลูกค้ารายไหนก็ได้ (และไม่มี audit trail ว่าใครเพิ่ม)

### ช่อง itype: เปลี่ยนความหมาย + ใบที่ขึ้นต้นด้วย W ต้องเลือก — 29/08/2569

~~ยังทำแค่ UI — ค่าที่เลือกไม่ได้บันทึกลง DB~~ → **บันทึกได้แล้ว 12/09/2569** ลงคอลัมน์ใหม่ **`morder.itype`** varchar(5) (migration `2026_09_12_100000_add_itype_to_morder_table` — **รันแล้ว** ด้วย `--path` เฉพาะไฟล์นี้) เก็บ key จาก config · `headerPayload()` รับเฉพาะ key ที่มีใน `config('order.itypes')` (ไม่ติ๊ก/ค่าแปลก = null) · `orderPayload()` คืน `itype` → `fillOrderForm()` ติ๊ก `#o_itype_<key>` คืน · `saveOrder()` ส่ง `itype` = ค่าของ `.o-itype-opt:checked`
...
- **key ใน config = ค่าที่เก็บลง `morder.itype` — ห้ามเปลี่ยน/ใช้ซ้ำ**
- ใบเก่าก่อน 12/09/2569 มี `itype` = NULL ทั้งหมด (เดิมไม่มีที่เก็บ)
...
`OrderController::checkPriceFloor()` เรียกก่อนเข้าทรานแซกชันใน `save()` — **บังคับทั้ง `insert` และ `update`** · 🟢 **ยกเว้นใบที่อนุมัติแล้ว ซึ่งข้ามด่านนี้ทั้งหมด** (21/09/2569 — ราคาขายถูกล็อกไม่ให้แก้แล้ว ดูหัวข้อ "กติกาการบันทึก"):
...
| ไม่มีราคาอนุมัติ · **รหัสมีใน `zcolorrate`** · ราคาขาย ≥ ราคาของกลุ่ม (A/B/C ตามน้ำหนักรวม) | ผ่าน |
| ไม่มีราคาอนุมัติ · **รหัสมีใน `zcolorrate`** · ราคาขาย < ราคาของกลุ่ม | **422** + `price_blocked: true` |
| ไม่มีราคาอนุมัติ · ไม่มีในกลุ่มราคา · ราคาขาย ≥ ราคาช่อง 2 | ผ่าน |
| ไม่มีราคาอนุมัติ · ไม่มีในกลุ่มราคา · ราคาขาย < ราคาช่อง 2 | **422** + `price_blocked: true` |
| ไม่กรอกราคาขาย (แต่คำนวณเกณฑ์ได้) | **422** |

- **ลำดับเกณฑ์: "ราคาอนุมัติ" (ถ้ายังยืนราคาอยู่) → "ราคาของกลุ่มราคา" (ถ้ารหัสมีใน `zcolorrate`) → ราคาช่อง 2** (`approvedFloor()` 28/08/2569 · `colorRateFloor()` 12/09/2569 — ทั้งคู่ตามที่ผู้ใช้สั่ง)
  - **ราคาของกลุ่มราคา = `zcolorrate.rate_A` / `rate_B` / `rate_C`** ของรหัสสินค้า เลือกคอลัมน์ตามกลุ่มที่ได้จาก **น้ำหนักรวม (`morder.netqty`)** ⇒ `save()` ต้องส่ง `netqty` เข้า `checkPriceFloor()` ด้วย · ตัวเดียวกับช่อง **"ขั้นต่ำ"** ที่โชว์ในกล่องราคา
  - **ไม่กรอกน้ำหนักรวม = หากลุ่มไม่ได้ ⇒ ตกไปใช้ราคาช่อง 2** (เกณฑ์อ่อนกว่า) · รหัสไม่มีในตาราง หรือค่าในกลุ่มนั้นเป็น 0/ว่าง ก็เช่นกัน
  - ⚠ ตาราง `zcolorrate` มีแค่ **28 รหัส** (25 รหัสเคยใช้จริงในใบสั่ง) ⇒ เกณฑ์นี้มีผลกับรหัสกลุ่มนี้เท่านั้น
  ...
  - `checkPriceFloor()` แนบ `floor_from` (`approved` / **`color_rate`** / `price_2`) มาใน response และข้อความ error เรียกชื่อเกณฑ์ให้ตรงกับที่ใช้จริง ("ราคาอนุมัติ" / "ราคาขั้นต่ำของกลุ่มราคา" / "ราคาช่อง 2") · `priceData()` คืน `min_from` ชุดเดียวกันให้ช่อง "ราคาต้องไม่ต่ำกว่า" บนจอ (มี `title` บอกที่มา) ⇒ ตัวเลขบนจอกับด่านตอนบันทึกตรงกันเสมอ
...
**กรอกรหัสลูกค้าที่ติด Blacklist = เตือนแล้วล้างรหัสทิ้ง** (19/09/2569 ตามที่ผู้ใช้สั่ง)

- **เกณฑ์: `customer.black` ไม่ใช่ 0** → `OrderController::isBlacklisted()` (private static) · **`NULL` ถือว่าไม่ติด** (1 ราย — ⚠ ยังไม่ได้ยืนยันกับผู้ใช้ ตั้งสมมติฐานไว้ว่าข้อมูลว่าง ≠ ขึ้นบัญชีดำ)
- `customerPayload()` แนบ **`is_black` · `blackrem` · `blackdate`** มากับ `GET order/customer/{code}`
- ฝั่งจอ (`lookupOrderCustomer` ใน `order/index.blade.php`): เจอ `c.is_black` → เรียก **`orderBlacklistBlock()`** แล้ว **`return` ก่อนเติมค่าใด ๆ** ⇒ ไม่มีข้อมูลของลูกค้ารายนั้นหลุดขึ้นฟอร์มเลย
- Swal แบบ error (มี `allowOutsideClick: false`) โชว์ รหัส + ชื่อ + เหตุผล (`blackrem`) + วันที่ขึ้นบัญชี (`blackdate`) → กด **ตกลง** แล้ว **ล้าง `#o_Custno` + คืนโฟกัสให้ช่องนั้น**
- ⚠ **ต้องล้างช่องอื่นเองด้วย** ผ่าน **`clearOrderCustomerFields()`** (ชื่อลูกค้า · สถานที่ส่ง · รหัสผู้ขาย · RP/CER/MSDS · กล่องราคา) — เพราะ `.val('')` **ไม่ยิง event `input`** ⇒ `lookupOrderCustomer()` ไม่ถูกเรียกซ้ำ ช่องอื่นจึงไม่ถูกล้างให้เอง
- ⚠ Swal ตั้ง `target: #orderModal` + `heightAuto: false` (แพทเทิร์นเดียวกับ Swal อื่นใน modal นี้)
- ⚠ **เป็นการกันฝั่งจอล้วน** — `OrderController::save()` **ไม่ได้ตรวจ blacklist** ⇒ ยิง POST ตรงยังบันทึกใบให้ลูกค้าที่ติด blacklist ได้ · ใบเก่าที่บันทึกไว้แล้วก็เปิดขึ้นมาแก้ได้ตามปกติ (`fillOrderForm` ไม่ได้เรียก `lookupOrderCustomer`)
- ⚠ **กระทบลูกค้า 3,005 จาก 4,094 ราย (73%)** ตามเกณฑ์ `black <> 0` — ส่วนใหญ่คือกลุ่ม `black = -3` (2,734 ราย) ที่ **ไม่มี `blackrem` เลยสักราย** (ดูหัวข้อ Blacklist ในส่วนฐานข้อมูลลูกค้า)
...
| สถานที่ส่ง | `morder.DVpoint` — ตัวเลือกจาก `naddress` (Custno + DVpoint) · **เพิ่มตัวเลือกใหม่ได้จากฟอร์มนี้เลย** (19/09/2569 ดูหัวข้อถัดไป) |
...
| ตารางรายการ: รหัสสินค้า / **รหัส** / ชื่อสินค้า / Lotno. | `suborder.Itemno` / **`nold`** (`O` / `N`) / `prodname` / `Lotno` — ⚠ บั๊กเดิม (แก้ 12/09/2569): `itemPayload()` ไม่ส่ง `nold` กลับ เปิดใบแล้วช่องรหัสว่าง และบันทึกซ้ำจะทับเป็นค่าว่าง |
...
**ห้ามวางข้อมูล (paste) ในช่อง "เลขที่ใบส่ง" และ "หมายเหตุ" ของตารางรายการ** (19/09/2569 ตามที่ผู้ใช้สั่ง) — delegated handler `$(document).on('paste drop', OI_NO_PASTE, ...)` ใน `order/index.blade.php` (`OI_NO_PASTE` = `[data-f="outno"]`, `[data-f="Remark"]`) → `preventDefault()` แล้วเด้ง toast "ช่องนี้ไม่อนุญาตให้วางข้อมูล"
- event **`paste`** ครอบทั้ง **Ctrl+V / Cmd+V และคลิกขวา → วาง**
- ⚠ **บล็อก `drop` ด้วย** ไม่งั้นลากข้อความมาปล่อยใส่ช่องก็ยังเข้าได้ (เลี่ยงด่าน paste ได้)
- ⚠ **ใช้ delegated event ที่ `document`** เพราะแถวถูกสร้างใหม่ทุกครั้งด้วย `orderItemRow()` — ผูกตรง element จะหลุดเมื่อเพิ่มแถว/โหลดใบใหม่
- ⚠ **กันได้แค่ฝั่งจอ** — `OrderController::save()` ไม่ได้ตรวจ ยิง POST ตรงยังส่งค่าอะไรมาก็ได้
- ช่องอื่นในตารางยังวางได้ตามปกติ (รหัสสินค้า / ชื่อสินค้า / Lotno / S / P / ช่องวันที่)
...
**น้ำหนักรวม (`morder.netqty`) คิดจากน้ำหนักในตารางรายการให้อัตโนมัติ** (`recalcOrderTotals(syncNet)` ใน `order/index.blade.php`, 14/09/2569) — `netqty` = **ผลรวม S (`suborder.Stock`) + P (`Production`) ของทุกแถว** (ตรงกับข้อมูลจริง **3,218 จาก 3,481 ใบ = 92.4%**; ที่ไม่ตรงส่วนใหญ่เป็นใบที่ `netqty` ว่าง หรือกรอกน้ำหนักรวมไว้แต่ยังไม่มีรายการ)
- ฟังก์ชันเดิมที่คิดยอดรวมท้ายตาราง (`#o_total_stock` / `#o_total_prod`) ทำต่อให้ 2 อย่าง: เติม `#o_netqty` แล้วเรียก **`refreshOrderPrice()`** ให้กล่องราคา (กลุ่มราคา A/B/C · ขั้นต่ำ · ราคาต้องไม่ต่ำกว่า) คำนวณใหม่ตามน้ำหนักที่เปลี่ยน
- จุดที่ทำให้อัปเดต: พิมพ์ในช่อง S/P (`oninput` ของช่อง `num()` ในแถว) · กดลบแถว (`removeOrderItem`) — เพิ่มแถวใหม่เป็นแถวว่างจึงไม่ต้องคำนวณ
- ⚠ **`renderOrderItems()` เรียกด้วย `recalcOrderTotals(false)`** = ตอนโหลดใบเดิมไม่คำนวณทับ **คงค่า `netqty` ที่บันทึกไว้** (แพทเทิร์นเดียวกับ `weight_produced` ในฟอร์ม Planning) ⇒ ใบเก่าที่ค่าไม่ตรงผลรวม (263 ใบ) จะถูกคำนวณใหม่**เฉพาะเมื่อผู้ใช้ไปแก้ช่อง S/P**
- **client-side ล้วน** ไม่แตะ `OrderController::save()` (ค่าที่บันทึกยังมาจากช่อง `#o_netqty` เหมือนเดิม) · ช่องนี้**ยังพิมพ์ทับเองได้** (ไม่ได้ทำ readonly) แต่จะถูกเขียนทับเมื่อแก้ S/P ต่อ
...
**ช่องในตารางรายการโตตามข้อความที่พิมพ์** (`oiAutoGrow()` / `oiAutoGrowAll()` ใน `order/index.blade.php`, 28/08/2569) — วัดความกว้างข้อความจริงด้วย canvas `measureText` แล้วตั้ง **`min-width` แบบ inline** ที่ตัว input (จงใจไม่ตั้ง `width` เพราะ `form-control` เป็น `width:100%` อยู่แล้ว — ตั้ง min-width ทำให้ `<td>` ถูกบังคับให้กว้างตาม โดย input ยังเต็มช่องเหมือนเดิม) ขอบเขต `OI_MIN_W` 80px – `OI_MAX_W` 420px
  ...
  - ผลที่เห็นถูกจำกัดด้วย 2 อย่าง: `maxlength="20"` ของช่อง Itemno/prodname/Lotno/outno (ตามขนาดคอลัมน์จริง ดู `clampToColumns()`) และ `min-width` ที่ `<th>` ใน `order/form.blade.php` ⇒ คอลัมน์**ขยายได้แต่ไม่หดต่ำกว่าค่า min-width นั้น**
  - 🔴 **`<th>` ที่ตั้งกว้างเกินข้อความจริง = คอลัมน์ "ดูเหมือนไม่ขยาย"** (19/09/2569) — ผู้ใช้แจ้งว่าช่อง **เลขที่ใบส่ง** กับ **หมายเหตุ** ไม่ขยายเหมือนช่องรหัสสินค้า **แต่ JS ผูกให้อยู่แล้วทั้งคู่** (ทั้งสองช่องสร้างด้วย `txt()` จึงมี class `oi-input` และ `applyItemLookup()` ก็เรียก `oiAutoGrow($rem[0])` ให้ตอนเติมหมายเหตุอัตโนมัติ)
    - สาเหตุจริงคือ **`<th>` ตั้งพื้นไว้กว้างเกินข้อความที่กรอกจริง** — หมายเหตุเคยเป็น **260px** ⇒ ต้องพิมพ์เกิน ~35 ตัวอักษรถึงจะเห็นขยาย · เลขที่ใบส่ง 110px
    - แก้แล้วโดย **ลด min-width**: เลขที่ใบส่ง `110px → 90px` · หมายเหตุ `260px → 150px` (ไม่ได้แตะ JS เลย)
    - ⚠ **เวลาเจออาการ "ช่องไม่ขยาย" ให้ดู `<th>` ก่อน** อย่าเพิ่งไปแก้ `oiAutoGrow()`
    - เพดานบนยังเป็น `OI_MAX_W` = 420px เหมือนเดิม (หมายเหตุยาวมากจะหยุดที่ 420 แล้วเลื่อนอ่านในช่องแทน)
...
ช่อง "กลุ่มราคา" (A/B/C จาก `netqty`) ยังแสดงอยู่แต่**ไม่ได้ใช้คิด "ราคาต้องไม่ต่ำกว่า"** — ใช้หาค่าช่อง "ขั้นต่ำ" (ด้านล่าง)
...
**ป้ายเกณฑ์กลุ่ม A/B/C ใต้ช่อง "จำนวนสั่งซื้อ" ในฟอร์ม MK** (12/09/2569 ตามฟอร์ม Access เดิม): 3 กล่อง `.pa-gitem` (`#a_glabel1/2/3`) ใน `price-approval.blade.php` — **ข้อความ hard-code ให้ตรงกับ `PRICE_GROUPS`** (ถ้าแก้เกณฑ์ในคอนโทรลเลอร์ ต้องมาแก้ข้อความตรงนี้ด้วย) · `highlightPriceGroup()` ใส่ `.pa-active` ให้ทั้งกล่องราคา (`.pa-pricebox`) และป้ายของกลุ่มที่ตรงกับจำนวนสั่งซื้อ · CSS อยู่ใน `order/index.blade.php`
- **แต่ละป้ายโชว์ "ราคาของกลุ่มนั้น" บรรทัดล่างด้วย (19/09/2569 ตามที่ผู้ใช้สั่ง)** — โครงป้ายเปลี่ยนจาก `<div>` ข้อความล้วน เป็น 2 `<span>`: `.pa-gitem-cap` (ข้อความเกณฑ์เดิม) + **`.pa-gitem-price` (`#a_gprice1/2/3`)**
  - **A = ราคาขาย 1 · B = ราคาขาย 2 · C = ราคาขาย 3** — ตัวเลข**ชุดเดียวกับช่องราคา 3 ช่อง**ด้านบน ไม่ได้ยิง endpoint เพิ่ม
  - 🔴 **เปลี่ยนแล้ว 26/09/2569: ป้ายโชว์ `zcolorrate.rate_A` / `rate_B` / `rate_C` อย่างเดียว — รหัสที่ไม่มีในกลุ่มราคา = ขีด `—`** (ผู้ใช้แจ้งว่าป้ายไม่ตรงกับเมนู "ดูกลุ่มราคา" และรหัสที่ไม่มีกลุ่มราคาไม่ควรมีตัวเลข) ⇒ ข้อความ "A = ราคาขาย 1 …" ด้านบนใช้ไม่ได้แล้ว · `PriceApprovalController::data()` แนบ key **`color_rate`** (null เมื่อไม่มีในตาราง) → `fillApprovalForm()` เรียก `setApprovalGroupPrices()` · `fillApprovalPrices()` **เลิกเรียก** `setApprovalGroupPrices()` แล้ว · **ช่องราคา 3 ช่องด้านบนยังเป็นราคาขาย 1/2/3 เหมือนเดิม** (แก้เฉพาะป้าย)
  - เติมด้วย **`setApprovalGroupPrices(p1, p2, p3)`** ใน `order/index.blade.php` เรียก 3 ที่: `fillApprovalPrices()` **ขาคำนวณได้** (`p.price_1/2/3` จาก `ProductPriceService`) · **ขาคำนวณไม่ได้** (`req.price1/2/3` = สำเนาราคาที่บันทึกไว้ใน `appvreq` — ให้ตรงกับช่องราคาที่โชว์ค่าเดิมแทน) · `clearApprovalForm()` (ล้างเป็น `—`)
  - ⚠ **ต้องล้างเองใน `clearApprovalForm()`** เพราะป้ายเป็น `<span>` ไม่ใช่ `<input>` จึงไม่โดน selector `#approvalModal input[type="text"]` ที่ล้างช่องอื่น ๆ
  - ไม่มีค่า = ขีด `—` (ไม่ปล่อยว่าง ป้ายจะได้ไม่เตี้ยลงสลับไปมา) · กลุ่มที่ถูกเน้น (`.pa-active`) ราคาเปลี่ยนเป็นสีน้ำเงินตาม

**ช่อง "ขั้นต่ำ" (`#o_min_qty`) = ค่าของกลุ่มราคาจากตาราง `zcolorrate`** (12/09/2569): `priceData()` หาแถว `zcolorrate.colorno` = รหัสสินค้า แล้วเอา `rate_A` / `rate_B` / `rate_C` ตามกลุ่ม (จาก `netqty`) → คืน `min_rate` → `fillPriceBox()` เติมลงช่อง · อ่านอย่างเดียว ไม่ได้บันทึก · ไม่มีน้ำหนักรวม หรือรหัสไม่มีใน `zcolorrate` = ว่าง (ตารางมีแค่ 28 รหัส) · แก้น้ำหนักรวมแล้วคำนวณใหม่ทันที (`oninput="refreshOrderPrice()"`)
...
**ตัวเดินระเบียน "ใบที่รออนุมัติ" บนหัวฟอร์ม** (12/09/2569): เปิดฟอร์มเปล่า (ไม่ได้ส่ง custno มา) → โหลด **ใบขอราคาที่ยังไม่อนุมัติทั้งหมด** แล้วเด้งเข้าใบแรกให้เลย · กดลูกศร `|< < [ช่อง] > >|` ไล่ดูใบอื่น (แถบอยู่ใน **`modal-footer` ของ `#approvalModal`** ใน `order/index.blade.php` ชิดซ้าย คู่กับปุ่มปิดที่ชิดขวา — ย้ายมาจากในฟอร์ม 12/09/2569 · พื้นหลังฟ้า class **`.pa-nav`** เข้าชุดกับ `.pa-body` · `#pa_pos` / `#pa_total` / `#pa_info` = ลูกค้า · เบอร์ · ชื่อลูกค้าของใบที่ดูอยู่)
- **endpoint:** `GET order/price-approval/pending` → `PriceApprovalController::pending()` — `appvreq` ที่ `Appv` เป็น NULL **หรือ 0** (Access เก็บ -1 = อนุมัติแล้ว) join `customer` เอาชื่อ เรียง `ReqDate` ใหม่→เก่า · ข้อมูลจริงตอนทำมี **3 ใบ**
- **badge จำนวนบนปุ่ม "ขออนุมัติราคาพิเศษ"** ของหน้ารายการ (`#paQueueBadge`, 12/09/2569) — `paLoadQueue()` อัปเดตให้ และถูกเรียกตั้งแต่ `$(function(){...})` ตอนเปิดหน้า จึงเห็นจำนวนโดยไม่ต้องเปิดฟอร์มก่อน (แพทเทิร์นเดียวกับ `#oaQueueBadge` ของปุ่มอนุมัติใบสั่งซื้อ) · ไม่มีใบรออนุมัติ = ซ่อน badge
- **ฝั่งจอ** (`order/index.blade.php`): `paLoadQueue()` / `paGo(i)` / `paStep(±1)` / `paRenderNav()` / `paRefreshQueue()` — `paGo()` แค่เติม `#a_custno` แล้วเรียก `onApprovalCustChange(custno, itemno)` ให้ flow เดิมโหลดทั้งฟอร์มต่อ (ไม่ได้เขียนการโหลดข้อมูลซ้ำ) · มิเรอร์แพทเทิร์นจากตัวเดินระเบียนของฟอร์มอนุมัติใบสั่งซื้อ (`oaGo`/`oaStep`)
- เรียกฟอร์มมาพร้อมคู่ (ลูกค้า, เบอร์) เช่นจากด่านราคาในใบสั่งซื้อ → เปิดคู่นั้นตามเดิม แถบยังโหลดคิวมาโชว์จำนวน (ตำแหน่งว่างไว้)
- **หลังบันทึก/ลบ เรียก `paRefreshQueue()`** โหลดคิวใหม่แล้วชี้กลับไปที่คู่เดิม — ใบที่เพิ่งอนุมัติจะหลุดจากคิว (ตำแหน่งว่าง) แต่ยังดูใบนั้นบนฟอร์มต่อได้

**ปุ่ม "Refresh" บนหัวฟอร์ม = เหมือนปิดแล้วเปิดฟอร์มใหม่** (12/09/2569 ตามที่ผู้ใช้สั่ง — เดิมเรียกแค่ `loadApprovalData()` โหลดข้อมูลของคู่ ลูกค้า+เบอร์ ที่ค้างอยู่ใหม่): `approvalRefresh()` เรียก **`approvalOpen()` ตัวเดิมแบบไม่ส่งลูกค้า** ⇒ ล้างฟอร์ม + ถามสถานะโหมดอนุมัติจาก server ใหม่ + โหลดคิวใบที่รออนุมัติแล้วเด้งเข้าใบแรก · `modal('show')` กับ modal ที่เปิดอยู่แล้วเป็น no-op จึงไม่กระพริบ · **มีม่านโหลด (overlay) คลุมทั้ง modal ระหว่างรีเฟรช** — `#approvalLoading` (CSS `.pa-loading` ใน `order/index.blade.php`, absolute คลุม `.modal-content`) เปิดด้วย `apvOverlay(true)` แล้ว **ปิดเองเมื่อ ajax ของหน้าเงียบติดกัน 2 รอบ (~600ms)** ผ่าน `apvWaitIdle()` — ⚠ ต้องเงียบ 2 รอบเพราะ `onApprovalCustChange` หน่วง 350ms ก่อนยิง `/items` ถ้าเช็ครอบเดียวม่านจะหายก่อนข้อมูลมา · มีเพดาน 20 วินาที + ปิดม่านตอน `hidden.bs.modal` กันม่านค้าง
...
🔴 **กับดักหลังอัพเดทราคาทุน 27/08/2569 — เปิดใบเก่าแล้วกดบันทึก = สำเนาราคาตอนที่ขอถูกทับ** เดิมใบเก่าส่วนใหญ่คำนวณสดไม่ได้ (ไม่มีรหัสใน `access_pdprice`) ฟอร์มจึงโชว์ค่าที่บันทึกไว้ใน `appvreq.price1/2/3` แล้วบันทึกทับด้วยค่าเดิม = ไม่เปลี่ยนอะไร. ตอนราคาทุนครบแล้ว **คำนวณสดได้เกือบทุกใบ ⇒ ฟอร์มโชว์ราคาปัจจุบัน และ `save()` (บรรทัด ~398) เขียนค่าที่โชว์นั้นลง `price1/2/3` ตรง ๆ** ⇒ MK เปิดใบเก่ามาแก้แค่หมายเหตุแล้วกดบันทึก สำเนาราคาตอนที่ขอก็หาย
  ...
  - **ยังไม่ได้แก้** — ถ้าต้องรักษาประวัติ ทางเลือกคือให้ `save()` เขียน `price1/2/3` เฉพาะตอนขึ้นใบใหม่ (ไม่เขียนตอนแก้ใบเดิม) หรือแยกช่อง "ราคาตอนที่ขอ" ออกจาก "ราคาปัจจุบัน" บนฟอร์ม — ทั้งใบเดิมและใบใหม่ ไม่ได้เอาค่าที่บันทึกไว้ใน `appvreq.price1/2/3` มาโชว์ (คอลัมน์นั้นกลายเป็น "สำเนาราคาตอนที่ขอ" เก็บไว้ดูย้อนหลัง — `save()` ยังเขียนค่าที่โชว์ลงไปเหมือนเดิม) · ค่าที่เคยบันทึกจะถูกหยิบมาแสดง **เฉพาะตอนคำนวณไม่ได้** (ไม่พบรหัสในตารางราคาทุน ฯลฯ) พร้อมข้อความบอกว่าเป็นค่าเดิม · ส่วน DB 3-4/1-2 Kg. คำนวณสด ๆ เสมอเช่นกัน (ไม่มีที่เก็บใน `appvreq`) · โค้ดอยู่ที่ `fillApprovalPrices()` ใน `order/index.blade.php`
  - 🔴 **บรรทัดบอกที่มาของราคาใต้กล่องราคา ถูกเอาออกแล้ว (19/09/2569 ตามที่ผู้ใช้สั่ง)** — เดิมโชว์ `ราคาทุน 79.00 · MB ทั่วไป (นอกจาก 3 เคสข้างบน) · × 108 ÷ 100 + 38` แบบเดียวกับใบสั่งซื้อ/หน้า `/saleinfo`. ตอนนี้ขาคำนวณได้จะเซ็ตข้อความเป็นค่าว่างแทน (**ต้องล้างข้อความ ไม่ใช่แค่ไม่เขียน** ไม่งั้นค่าของเบอร์ก่อนหน้าค้างบนจอ)
  - ⚠ **`#a_price_note` ยังอยู่ อย่าลบทิ้ง** — ขา "คำนวณไม่ได้" ยังใช้กล่องนี้บอกเหตุผล (เช่น "ไม่พบรหัสสินค้านี้ในตารางราคาทุน — แสดงราคาที่บันทึกไว้ในใบเดิมแทน") พร้อมกรอบเตือน `of-price-note-warn`
  - ⚠ **เอาออกเฉพาะฟอร์ม MK นี้** — บรรทัดบอกที่มาในฟอร์มใบสั่งซื้อ และหน้า `/saleinfo` **ยังอยู่เหมือนเดิม**

**ปุ่ม "พิมพ์" → รายงาน PDF "รายการที่อนุมัติราคาแล้วใน 3 วันล่าสุด"** (12/09/2569 — เดิมเป็น `window.print()` เพราะยังไม่มีแบบฟอร์มกระดาษให้อ้างอิง): `approvalPrint()` → `GET order/price-approval/approved-pdf` → `PriceApprovalController::approvedRecentPdf()` (mPDF A4 แนวตั้ง) → blade **`order/price-approval-approved-pdf.blade.php`**
- **ผัง = 1 ระเบียน 1 บล็อก** ตามกระดาษที่ผู้ใช้ส่งมา (ไม่ใช่ตาราง): บรรทัด 1 `เวลาอนุมัติ` · บรรทัด 2 `วันที่ขอ` + รหัสลูกค้า + ชื่อลูกค้า (`customer.name`) · บรรทัด 3 **รหัสสินค้า** + `ราคาขายครั้งนี้ <price> บาท` + `จำนวนสั่งซื้อ <weight> ก.ก.` (3 ค่านี้ตัวใหญ่หนา) · บรรทัด 4 หมายเหตุ (`remark`) + เส้นคั่น · วันที่รูปแบบ `d/m/y H:i` (ปี 2 หลัก) เรียงใหม่→เก่า
- **กรอง:** `Appv` ≠ 0 และไม่ null (Access เก็บ -1 = อนุมัติแล้ว) + **`AppvDT >= วันนี้ 00:00 - 2 วัน`** (21/09/2569 — เดิมกรองด้วย `ReqDate`)
- 🟢 **"เวลาอนุมัติ" = `appvreq.AppvDT` แล้ว (21/09/2569)** — เดิมไม่มีคอลัมน์นี้จึงใช้ `ReqDate` แทน (12/09/2569) · ได้มาพร้อม `MGTrem` ตอน**นำเข้า `appvreq` ชุดใหม่จาก DB ของลูกค้า** (ดูหัวข้อ "นำเข้า appvreq ชุดใหม่") · แก้ 3 จุด: `approvedRecentPdf()` (`$r->appv_at = $r->AppvDT` + กรอง/เรียงด้วย `AppvDT`) · `save()` (เขียนค่า ดูด้านล่าง) · คอมเมนต์หัว blade
  - **ต่างจากเดิมจริง ไม่ใช่แค่เปลี่ยนชื่อคอลัมน์** — ใบที่ขอวันหนึ่งแล้วอนุมัติอีกวันเคยตกหล่น เช่น ใบของลูกค้า `41047` ขอ 18/09 อนุมัติ 19/09: เกณฑ์เดิมได้ 4 รายการ เกณฑ์ใหม่ได้ 5 (ตรวจกับข้อมูลจริง 21/09/2569)
  - ⚠ **ใบที่ `AppvDT` ว่างไม่เข้ารายงาน** (NULL เทียบ `>=` ได้ NULL = ไม่ผ่าน) — **22,600 จาก 22,891 แถวเป็น NULL** เพราะระบบเดิมเพิ่งเริ่มเก็บ `AppvDT` เมื่อ **12/06/2569** · ใบเก่ากว่านั้นเกิน 3 วันอยู่แล้วจึงไม่กระทบรายงานนี้
- **`save()` เขียน `AppvDT` เอง (21/09/2569):** เพิ่งอนุมัติ (`$appv && !$alreadyApproved`) = ประทับ `now()` · ยกเลิกอนุมัติ (`!$appv`) = ล้างเป็น `null` · ⚠ **ใบที่อนุมัติไปแล้วแล้ว MK แก้หมายเหตุ/ราคาต่อ = ไม่แตะ** คงเวลาอนุมัติเดิมไว้ (ขานั้นช่องอนุมัติถูก disabled ค่าที่ส่งมาเป็นสถานะเดิม ไม่ใช่การอนุมัติใหม่) — ทดสอบครบ 4 เคสผ่าน controller จริงแล้ว 21/09/2569
- ⚠ **"3 วันล่าสุด" = 3 วันปฏิทินย้อนหลัง** (วันนี้ + 2 วันก่อนหน้า, ค่าคงที่ `PRINT_RECENT_DAYS`) ตามที่ผู้ใช้เลือก ⇒ ช่วงไหนไม่มีใครอนุมัติ รายงานจะขึ้น "ไม่พบรายการที่อนุมัติราคาใน 3 วันล่าสุด"
...
**ปุ่ม "ประวัติของเบอร์นี้" → เปิดรายงาน PDF** (12/09/2569 — เดิมโหลดผลลงตารางล่างในฟอร์ม): `approvalHistory()` เปิดแท็บใหม่ไปที่ `GET order/price-approval/history-pdf?custno=&itemno=` → `PriceApprovalController::historyPdf()` (mPDF **A4 แนวตั้ง** — เปลี่ยนจาก A4-L 12/09/2569 ตามที่ผู้ใช้สั่ง) → blade **`order/price-approval-history-pdf.blade.php`** · ผังตามรายงานกระดาษเดิม: หัว "ประวัติการขออนุมัติราคาของเบอร์ ... `<itemno>`" + "เรียงจากปัจจุบัน ==> อดีต" · 10 คอลัมน์ (วันที่ขอ `d/m/y` · รหัสลูกค้า · รหัสสินค้า · จน.สั่งซื้อ · ราคาที่ขอ · price1/2/3 · หมายเหตุการปรับราคา · อนุมัติ ✔) · **ข้อมูลชุดเดียวกับปุ่มประวัติ = คู่ (ลูกค้า, เบอร์) ที่เลือกอยู่เท่านั้น** · `price1/2/3` = ค่าที่บันทึกไว้ใน `appvreq` (สำเนาราคาตอนที่ขอ) ไม่ได้คำนวณสดเหมือนบนฟอร์ม · ยังไม่ได้เลือกลูกค้า/เบอร์ = เตือนแล้วไม่เปิด

**ปุ่ม "ประวัติ ราคาเม็ด CP" → เปิดรายงาน PDF** (12/09/2569 — เดิมโหลดผลลงตารางล่าง): `approvalResinHistory()` → `GET order/price-approval/resin-history-pdf?itemno=` → `PriceApprovalController::resinHistoryPdf()` (mPDF A4-L) → blade **`order/price-approval-resin-history-pdf.blade.php`** · **เฉพาะเบอร์นี้ แต่ของลูกค้าทุกคน** — `cp_itemprice` ผูกกับเลขที่ใบสั่ง **ไม่มีคอลัมน์ลูกค้า** จึงไม่ต้องกรอง custno · **พิมพ์ทุกแถว** (ตารางบนจอเดิมจำกัด 50 แถว)
- **ผัง = 1 ระเบียน 1 บล็อก 3 คอลัมน์แบบ "ป้าย: ค่า"** ตามกระดาษเดิม (ไม่ใช่ตาราง) — หัวรายงาน "ประวัติราคาเม็ด" + "เรียงจากปัจจุบัน ==> อดีต  บรรทัดแรก เป็นการขอราคาครั้งนี้" · ฟิลด์ในบล็อก: `Mdate` `itemno` `wage` `Resin1Code` `Resin2Code` `wageCal` | `Orderno` `OrderPrice` `ResinFrom` `Resin1Price` `Resin2Price` `Diff` | `Custno` `Qdate` `Resin1Per` `Resin2Per` `status`
- ⚠ **`Mdate` / `Custno` ไม่มีใน `cp_itemprice`** → `leftJoin morder` ด้วยเลขที่ใบสั่ง · **247 จาก 482 แถวหาใบใน `morder` ไม่เจอ** (ใบเก่า) 2 ช่องนั้นจึงเว้นว่าง และเรียงด้วย `COALESCE(m.Mdate, c.Qdate) DESC` ไม่งั้นแถวที่ไม่มีใบจะหล่นไปท้ายสุดผิดลำดับเวลา
- ตัวเลขแสดงแบบตัดศูนย์ท้าย (6.7 / 59.5 / 66.27) ตามกระดาษ · `Mdate` = `d/m/y g:i:s A` · `Qdate` = `d/m/y`

**ปุ่ม "ตรวจสอบ เบอร์อื่น ..." → ถามรหัสสินค้า แล้วเปิดรายงาน PDF** (12/09/2569 — เดิมโหลด `zcustprice` ทุกเบอร์ของลูกค้าลงตารางล่าง): `approvalOtherItems()` เด้ง **Swal ที่มีช่องกรอกรหัสสินค้า** (ค่าเริ่มต้น = เบอร์ที่เลือกอยู่บนฟอร์ม, บังคับกรอก) → `GET order/price-approval/other-items-pdf?itemno=` → `PriceApprovalController::otherItemsPdf()` (mPDF **A4 แนวตั้ง** — ผู้ใช้สั่ง 12/09/2569 · "ประวัติของเบอร์นี้" ก็เปลี่ยนเป็นแนวตั้งตามในวันเดียวกัน จึงเหมือนกันทั้งคู่)
- **ใช้ blade ตัวเดียวกับ "ประวัติของเบอร์นี้"** (`order/price-approval-history-pdf.blade.php`) — ผัง/คอลัมน์/**หัวรายงานเหมือนกันเป๊ะ** ตามที่ผู้ใช้สั่ง (เคยลองเติม "(ทุกลูกค้า)" ต่อท้ายหัวรายงาน + ตัวแปร `$heading` แต่**ผู้ใช้สั่งให้เอาออก 12/09/2569** จึงกลับไปใช้หัวข้อความเดียวกันทั้ง 2 ปุ่ม)
- **ต่างจาก `historyPdf()` ตรงที่ไม่กรอง `custno`** — ดึง `appvreq` ของเบอร์ที่กรอก **ทุกลูกค้า ทั้งใบที่อนุมัติแล้วและยังไม่อนุมัติ** เรียง `ReqDate` ใหม่→เก่า แล้ว `custno` · คอลัมน์ "รหัสลูกค้า" มีอยู่ในผังเดิมอยู่แล้วจึงอ่านออกว่าแถวไหนของใคร
- **ช่อง "อนุมัติ" เป็นกล่องติ๊กแบบฟอร์มกระดาษ (12/09/2569)** — วาดกล่องทุกแถว (ยังไม่อนุมัติ = กล่องว่าง, อนุมัติแล้ว = ✔) ด้วย **ตารางซ้อน 1 ช่อง `table.chk`** ไม่ใช่ `span` + `display:inline-block` เพราะ mPDF คุมความสูง/เส้นขอบของ inline-block ได้ไม่แน่นอน · 🔴 **เครื่องหมายถูกต้องบังคับฟอนต์ `dejavusanscondensed`** (class `.chk-mark`) — ฟอนต์ไทยที่ mPDF เลือกใช้เองคือ **Garuda ซึ่งไม่มี glyph U+2714/U+2713/U+2717** (ตรวจ cmap แล้ว) ปล่อยไว้จะได้กล่องทึบ (tofu) แทนเครื่องหมายถูก — เจอตอนผู้ใช้ทักว่า "ยังเพี้ยนอยู่" 12/09/2569
- **ขนาดฟอนต์ใน blade ลดลงทั้งชุด 12/09/2569** (body 12→10px · ช่องตาราง 11→9px + padding 5/6→4/5px · หัวรายงาน 16→14px · ราคาที่ขอ 14→11px) — ⚠ blade ใช้ร่วมกัน 2 ปุ่ม ⇒ **"ประวัติของเบอร์นี้" เล็กลงตามไปด้วย**
- ⚠ **Swal ต้องตั้ง `target: #approvalModal`** — ไม่งั้น focus trap ของ Bootstrap modal ดึงโฟกัสกลับ พิมพ์ในช่องไม่ได้ (คู่กับ `heightAuto:false` กันหน้าเลื่อน)
- ⚠ **ต้องกรอกรหัสสินค้าเสมอ (เว้นว่างไม่ได้)** — `appvreq` มี 22,339 แถว / 2,143 เบอร์ ถ้าพิมพ์ "ทุกสินค้า" ในไฟล์เดียวจะได้ PDF หลายร้อยหน้า · เบอร์ที่มีประวัติมากสุด 432 แถว (~10 หน้า) · ถ้าภายหลังต้องการแบบรวมทุกเบอร์จริง ๆ ให้เพิ่มเงื่อนไขที่ `otherItemsPdf()` จุดเดียว
- endpoint JSON เดิม `/other-items` + `ZCUST_COLS` ยังอยู่ แต่ไม่มีคนเรียกแล้ว (คอมเมนต์กำกับไว้)

**ปุ่ม "ตรวจสอบเฉพาะร้าน ..." → เปิดรายงาน PDF** (12/09/2569 — เดิมโหลด `zcustprice` ของลูกค้ารายอื่นที่ใช้เบอร์นี้ลงตารางล่าง): `approvalOtherCustomers()` → `GET order/price-approval/customer-history-pdf?custno=` → `PriceApprovalController::customerHistoryPdf()` (mPDF A4 แนวตั้ง)
- **ลูกค้ารายที่เปิดอยู่ ทุกเบอร์ ทั้งอนุมัติและไม่อนุมัติ** = คู่ตรงข้ามของ `otherItemsPdf()` (เบอร์เดียว ทุกลูกค้า) · ใช้ blade `order/price-approval-history-pdf.blade.php` ตัวเดียวกันทั้ง 3 ปุ่ม · ไม่ต้องกรอกอะไร (ใช้ `#a_custno` บนฟอร์ม, ยังไม่เลือกลูกค้า = เตือนแล้วไม่เปิด) · เรียง `ReqDate` ใหม่→เก่า แล้ว `itemno`
- **ตัวใหญ่กลางหัวรายงาน = ค่าที่ใช้กรอง** — blade รับ `$doc_subject` (default = `$itemno`): ปุ่มประวัติ/เบอร์อื่นส่งรหัสสินค้า · ปุ่มเฉพาะร้านส่งรหัสลูกค้า
- 🔴 **ต้องขยาย `pcre.backtrack_limit` ก่อน `WriteHTML()`** — ลูกค้ารายใหญ่สุดตอนนี้มี **1,896 แถว = HTML ~1.4 MB** ซึ่งเกิน default 1,000,000 ⇒ mPDF โยน `MpdfException` ทันที (เจอจริงตอนทดสอบลูกค้า `36017`) · เมธอดนี้จึง `ini_set('pcre.backtrack_limit', '10000000')` ให้เอง · รายงานอีก 2 ใบยังไม่เจอเพราะแถวน้อยกว่ามาก (เบอร์ที่มีประวัติเยอะสุด 432 แถว)
- ⚠ **ใบใหญ่สุดใช้เวลา ~14 วินาที · peak memory ~368 MB · 34 หน้า** (เครื่อง dev: `memory_limit` 512M, `max_execution_time` 120s ⇒ ผ่าน) — ถ้า server ของลูกค้าตั้งต่ำกว่านี้อาจได้ 500/timeout ต้องขยาย ini ที่ server หรือใส่ `ini_set` เพิ่มในเมธอดนี้
- endpoint JSON เดิม `/other-customers` + `OTHERCUST_COLS` ยังอยู่ แต่ไม่มีคนเรียกแล้ว

⚠ ตอนนี้ **ปุ่มตรวจสอบ/ประวัติทั้ง 4 ปุ่มเปิดเป็น PDF หมดแล้ว** — ตารางล่างในฟอร์ม (`#a_gridBody`) เหลือไว้แสดง "ราคาที่ยืนไว้ของเบอร์นี้" ตอนเลือกลูกค้า+เบอร์เท่านั้น · endpoint JSON `/other-items` `/other-customers` `/history` `/resin-history` + `HISTORY_COLS` / `RESIN_COLS` / `OTHERCUST_COLS` ยังอยู่ครบแต่ไม่มีคนเรียกแล้ว (มีคอมเมนต์กำกับไว้ เผื่อกลับมาใช้)

ปุ่มตรวจสอบ/ประวัติทั้ง 4 ปุ่ม (ก่อน 12/09/2569) โหลดผลลงตารางล่างตัวเดียวกัน: **ตรวจสอบเบอร์อื่น** → `zcustprice` ทุกเบอร์ของลูกค้า · **ประวัติของเบอร์นี้** → `appvreq` ทุกครั้งของคู่ custno+itemno · **ตรวจสอบเฉพาะร้าน** → `zcustprice` ของลูกค้ารายอื่นที่ใช้เบอร์นี้ · **ประวัติราคาเม็ด CP** → `cp_itemprice` ของเบอร์นี้


### นำเข้า `appvreq` ชุดใหม่จาก DB ของลูกค้า — 21/09/2569

ลูกค้าส่ง dump ของ DB ที่ใช้งานจริง (`Source Database: Orange`) มาให้ — ไฟล์อยู่ที่ **`public/sql/`** (`appvreq` · `morder` · `suborder` · `uprice` · `pdprice` · `orderrun` · `target`) · **นำเข้าแล้วเฉพาะ `appvreq`** ลง `zwek_worldpigment_2` (DB ที่ `.env` ชี้อยู่ — ก่อนหน้านี้**ยังไม่มีตารางนี้เลย**)

- 🔴 **import ตรง ๆ ไม่ผ่าน — `#1067 Invalid default value for 'ReqDate'`** เพราะ dump มาจาก MySQL รุ่นเก่าที่ยอมให้มี zero-date แต่ MariaDB 10.4 ของ XAMPP ตั้ง `sql_mode = NO_ZERO_IN_DATE,NO_ZERO_DATE` ⇒ **ต้องผ่อน `sql_mode` เฉพาะ session ที่ import**
  ```bash
  mysql -uroot --default-character-set=utf8 --init-command="SET SESSION sql_mode=''" zwek_worldpigment_2 < public/sql/appvreq.sql
  ```
  (หรือแทรก `SET SESSION sql_mode='';` ไว้บนสุดของไฟล์ก่อน `CREATE TABLE`) · ⚠ **ผ่าน phpMyAdmin อาจไม่ได้ผล** เพราะมันตั้ง sql_mode ของตัวเองทับ — ให้ใช้ command line · ไฟล์เป็น **UTF-8 ไม่มี BOM** และ DB ปลายทาง default `utf8_unicode_ci` ⇒ ภาษาไทยเข้าครบ (ตรวจด้วย `HEX()` เทียบกับไบต์ในไฟล์แล้ว)
- **2 คอลัมน์ที่ได้เพิ่มมาจากของเดิม:** **`AppvDT`** datetime (เวลาที่กดอนุมัติ — เอาไปใช้แล้ว ดูหัวข้อปุ่ม "พิมพ์") และ **`MGTrem`** varchar(100) (หมายเหตุผู้บริหาร — **ยังไม่มีโค้ดไหนอ่าน/เขียน**, มีข้อมูลจริง 26 แถว ส่วนใหญ่ซ้ำกับท้ายข้อความใน `remark`)
- **แปลง `AppvDT` ที่เป็น `'0000-00-00 00:00:00'` → `NULL` แล้ว 22,543 แถว** (ค่าพวกนี้แปลว่า "ไม่เคยเก็บ" อยู่แล้ว และถ้าปล่อยไว้ Carbon จะ throw ตอนโค้ดอ่าน) ⇒ ตอนนี้ **22,891 แถว · มี `AppvDT` จริง 291 แถว** (12/06/2569–ปัจจุบัน)
- 🔴 **dump ไม่มีคอลัมน์ `costup`** ที่ฟอร์ม MK ใช้ ⇒ **ต้องรัน migration ต่อทุกครั้งหลัง import ทับตารางนี้** ไม่งั้น `PriceApprovalController::save()` พังทันที (มันเขียน key `costup` เสมอ)
  ```bash
  php artisan migrate --path=database/migrations/2026_08_20_100000_add_costup_to_appvreq_table.php
  ```
  (idempotent + ผ่อน sql_mode ให้เองอยู่แล้ว — **รันแล้ว** บน `zwek_worldpigment_2`)
- **`ReqDate` ยังมี `DEFAULT '0000-00-00 00:00:00'` ติดมาตามต้นทาง** (จงใจไม่แก้ ให้ตรงกับ DB ลูกค้า) ⇒ **`ALTER TABLE` ตารางนี้ครั้งหน้าก็ต้องผ่อน `sql_mode` อีก** เหมือนที่ migration `costup` ทำ
- ⚠ **`zwek_worldpigment` (DB เดิม) ไม่ได้ใช้แล้ว** — มี `appvreq` แบบเก่า (ไม่มี `AppvDT`/`MGTrem`) และ migrations แค่ 3 แถว เทียบกับ 31 แถวใน `zwek_worldpigment_2`

🟢 **ไฟล์ใน `public/sql/` ถูกแก้ให้ตรงกับโครงสร้าง DB จริงแล้ว — 22/09/2569** ⇒ **import ซ้ำได้โดยคอลัมน์ที่ระบบเราเพิ่มไม่หายอีก** (เดิมต้องตามซ่อมทุกครั้ง ดูหัวข้อถัดไป)

| ไฟล์ | สิ่งที่แก้ |
|---|---|
| `appvreq.sql` | `CREATE TABLE` เพิ่ม **`costup`** (ตำแหน่งที่ 10 หลัง `remark`) |
| `morder.sql` | เพิ่ม **`itype`** (ท้ายสุด) |
| `uprice.sql` | เพิ่ม **`id` AUTO_INCREMENT + `PRIMARY KEY`** (ตำแหน่งแรก) |
| ทั้ง 3 ไฟล์ | **`INSERT` ทุกบรรทัดระบุรายชื่อคอลัมน์แล้ว** (`INSERT INTO \`t\` (\`a\`, \`b\`) VALUES …`) |

- 🔴 **ต้องระบุรายชื่อคอลัมน์ใน `INSERT` เสมอ — นี่คือหัวใจ** ของเหลือเดิมเป็น `INSERT INTO \`t\` VALUES (…)` ซึ่งอิง**ลำดับคอลัมน์**: พอ `CREATE` มีคอลัมน์เพิ่ม ค่าจะ **เลื่อนช่อง** (`uprice.id` อยู่ตำแหน่งแรก) หรือ **column count ไม่ตรง** ทันที · การระบุชื่อทำให้คอลัมน์ใหม่ได้ `NULL`/default เอง และไฟล์ยังทนต่อการเพิ่มคอลัมน์ในอนาคต
- ⚠ **ห้ามปล่อย `AUTO_INCREMENT=N` ติดมาจาก `SHOW CREATE TABLE`** — ถ้าคัดลอกมาทั้งก้อน `uprice.id` จะเริ่มนับที่ค่านั้น (เจอจริง: ได้ 60713..121424 แทน 1..60712) ⇒ ถอดออกก่อนเสมอ
- **CREATE TABLE ในไฟล์ = ผลของ `SHOW CREATE TABLE` จาก `zwek_worldpigment_2`** จึงได้ index + `DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci` ครบ (ไฟล์เดิมไม่ระบุ charset เลย)
- **ทดสอบแล้ว**: import ทั้ง 3 ไฟล์เข้า database `test` → โครงสร้างตรงกับ DB จริง**เป๊ะทุกตัวอักษร** · จำนวนแถวครบ · `costup`/`itype` เป็น NULL ทั้งหมด · `uprice.id` = 1..60712 ไม่ซ้ำ · ภาษาไทยตรงกับตารางจริง
- ไฟล์โตขึ้นจากการเติมชื่อคอลัมน์: `appvreq` 4.9→7.6 MB · `morder` 2.3→3.8 MB · `uprice` 11.6→19.1 MB (สำรองต้นฉบับไว้ใน scratchpad ของ session 22/09/2569 — **ไม่ได้เก็บในโปรเจกต์**)
- **อีก 4 ไฟล์ไม่ต้องแก้** (`orderrun` · `pdprice` · `suborder` · `target`) — โครงสร้างตรงกับ DB อยู่แล้ว และ `INSERT` ยังเป็นแบบไม่ระบุคอลัมน์ตามต้นฉบับ
- ⚠ **`appvreq.sql` ยังต้องการ `SET SESSION sql_mode='';`** (มีอยู่ในไฟล์แล้ว) เพราะค่า `0000-00-00` ใน `AppvDT` และ default ของ `ReqDate`

🔴 **import ทับตาราง legacy = คอลัมน์ที่ระบบเราเพิ่มไว้หายไป แต่ตาราง `migrations` ยังบอกว่า "รันแล้ว"** ⇒ `php artisan migrate` **จะไม่รันซ้ำให้** ต้องซ่อมมือ (ตรวจพบ 21/09/2569 — **แก้ที่ต้นเหตุแล้ว** ดูหัวข้อบน แต่เก็บวิธีซ่อมไว้เผื่อเจอไฟล์ dump ชุดใหม่)

| คอลัมน์ | migration | สถานะที่เจอ |
|---|---|---|
| `morder.itype` | `2026_09_12_100000_add_itype_to_morder_table` | หายไป — **ซ่อมแล้ว 21/09/2569** |
| `uprice.id` | `2026_08_29_100000_add_id_to_uprice_table` | หายไป — **ซ่อมแล้ว 21/09/2569** (60,712 แถว ได้ `id` 1..60712 ครบ) |
| `appvreq.costup` | `2026_08_20_100000_add_costup_to_appvreq_table` | ซ่อมแล้ว (ดูด้านบน) |

- **อาการของ `morder.itype` ที่หาย: บันทึกใบสั่งซื้อไม่ได้เลย** — `500 Unknown column 'itype' in 'field list'` ทุกใบ (ไม่ใช่แค่ใบที่อนุมัติ) เพราะ `headerPayload()` ใส่ key `itype` เสมอ
- **อาการของ `uprice.id` ที่หาย:** เมนู "กำหนดราคา" (`/saleinfo`) **แก้ไข/ลบราคาไม่ได้** — ตารางไม่มี PK ให้ระบุแถว (⚠ ALTER ตารางนี้ rebuild ทั้ง 60,603 แถว MyISAM **สำรองก่อน**)
- **วิธีซ่อม** — ลบแถวใน `migrations` แล้วรันเฉพาะไฟล์นั้น (migration ทุกตัวเป็น idempotent อยู่แล้ว):
  ```bash
  mysql -uroot zwek_worldpigment_2 -e "DELETE FROM migrations WHERE migration='<ชื่อ migration>';"
  php artisan migrate --path=database/migrations/<ไฟล์>.php
  ```
...
**ต้องกรอก "รหัสพนักงาน" ก่อนอนุมัติ (16/09/2569 ตามที่ผู้ใช้สั่ง — เดิม 12/09/2569 เป็นรหัสผ่าน):** กดติ๊ก "อนุมัติ" → Swal เดิม (ยืนยันพร้อมข้อมูลใบ) **มีช่อง `input: 'text'` ป้าย "รหัสพนักงาน"** (maxlength 4 ตามขนาดคอลัมน์) → ส่ง `empno` ไปกับ `POST order/order-approval/approve` → `OrderApprovalController::approveEmpnoError()` ตรวจ **ก่อนแตะ DB** ไม่ผ่าน = **422 + `bad_empno: true`**
- **ผ่านเมื่อรหัสที่กรอกตรงกับพนักงานที่ยังใช้งานอยู่** — `emp.empno` + `is_active = 'Y'` (ข้อมูลจริง: 33 คน active ทุกคน · `empno` varchar(4) ไม่ซ้ำ ไม่ว่าง) · ข้อความ 422 มี 2 กรณี: **กรุณากรอกรหัสพนักงาน** (ไม่ส่ง/ช่องว่าง) · **รหัสพนักงานไม่ถูกต้อง** (ไม่มีในตาราง หรือถูกปิดใช้งาน)
- **ประวัติการเปลี่ยนด่านนี้:** รหัสร่วมใน `config('order.md_password')` (12/09) → รหัสผ่านของบัญชีที่ล็อกอิน (12/09) → รหัสผ่านของ "ใครก็ได้" ทั้ง `emp.password` + `users.password` (16/09) → **รหัสพนักงาน (16/09, ปัจจุบัน)** ⇒ **ไม่มีการเทียบ `Hash::check` แล้ว** (ถอด import `Hash` + `AccessControl` ออกจากคอนโทรลเลอร์) และหายปัญหาเดิมที่รหัสผิดต้องวน bcrypt ครบทุก hash ≈ 1 วินาที
- ⚠ **ด่านนี้อ่อนกว่ารหัสผ่านมาก** — รหัสพนักงานเป็นข้อมูลที่คนในโรงงานเห็นกันทั่วไป ใครรู้ก็กดอนุมัติแทนได้ ⇒ เป็นแค่การ "ยืนยันตัวผู้กด" ตามที่ผู้ใช้ต้องการ ไม่ใช่การพิสูจน์ตัวตน
- ⚠ **ยังไม่บันทึกว่าใครอนุมัติ** — `morder` ไม่มีคอลัมน์ผู้อนุมัติ ⇒ รหัสที่กรอกถูกใช้ตรวจอย่างเดียวแล้วทิ้ง (ถ้าจะเก็บ: เพิ่มคอลัมน์ก่อน แล้วเขียนค่า `empno` ลงไปใน `approve()` จุดเดียว)
- ⚠ **ไม่มี session ปลดล็อกแบบฟอร์ม MK** — ต้องกรอกทุกครั้งที่กด (ตรงตามที่ผู้ใช้สั่ง "เมื่อกดอนุมัติ ให้ป้อนรหัสก่อน")
- **บังคับทั้งขาอนุมัติและขายกเลิกอนุมัติ** (เป็นการแก้สถานะเอกสารเหมือนกัน แม้ UI ยังเข้าไม่ถึงขายกเลิก)
- Swal ตัวนี้ตั้ง `target: #orderApprovalModal` + `heightAuto:false` — ไม่งั้น focus trap ของ Bootstrap modal ดึงโฟกัสกลับ พิมพ์รหัสไม่ได้ (แพทเทิร์นเดียวกับ Swal ที่มี input ในฟอร์ม MK)

⚠ **ความเสี่ยงที่ยังค้างอยู่** (เหมือน `POST order/save`): route นี้ป้องกันแค่ `auth` + สิทธิ์ `order read` จาก `config/menu.php` — **ไม่มีสิทธิ์แยกสำหรับการอนุมัติ** (รหัสพนักงานด้านบนเป็นด่านเดียว ซึ่งใครรู้รหัสของคนอื่นก็กดแทนได้) และ **ไม่มี audit trail ว่าใครกด** — `morder` ไม่มีคอลัมน์ผู้อนุมัติ
...
- **Blacklist แสดงอย่างเดียว** (กล่องแดงบนหัวฟอร์ม + กล่องสรุปท้ายฟอร์ม + badge ในตาราง) — `save()` ไม่แตะ `black`/`blackdate`/`blackrem` เพราะสถานะเครดิต/blacklist มีตารางขออนุมัติของตัวเองอยู่แล้ว (`appvcredit`)
  - 🔴 **เกณฑ์ที่ถูกต้อง: ติด Blacklist = `customer.black` ไม่ใช่ `0`** (ผู้ใช้แจ้ง 19/09/2569)
  - ❌ **เอกสาร/โค้ดเดิมผิด** — เคยเข้าใจว่า `-1` เท่านั้นคือ blacklist (ตาม convention checkbox ของ Access) แล้วถือว่า `-3`/`2`/`5` เป็นค่าที่ "ยังไม่ทราบความหมาย" จึงไม่นับ
  - ค่าที่มีจริง: `-3` **2,734 ราย** · `0` 1,088 · `-1` 220 · `2` 49 · `5` 2 · NULL 1 (รวม 4,094)
  - ✅ **ฟอร์มใบสั่งซื้อใช้เกณฑ์ใหม่แล้ว** (19/09/2569) — `OrderController::isBlacklisted()` กรอกรหัสลูกค้าที่ติด blacklist แล้วจะเตือน + ล้างรหัสทิ้ง (ดูหัวข้อใบสั่งซื้อ)
  - ⚠ **แต่ 3 จุดในเมนูฐานข้อมูลลูกค้ายังใช้ `= -1` อยู่ — ยังไม่ได้แก้ (รอผู้ใช้สั่ง)**: `CustomerController::applyFilters()` (ตัวกรอง "สถานะ") · `customer/table.blade.php` (badge) · `customer/customer-form.blade.php` (`$isBlack`) ⇒ **ตอนนี้ 2 เมนูใช้เกณฑ์ไม่ตรงกัน**
  - ⚠ แก้แล้วจำนวนที่ติด blacklist จะเปลี่ยนจาก **220 → 3,005 ราย (73%)** · กลุ่ม `-3` **ไม่มี `blackrem` เลยสักราย** (ต่างจาก `-1` ที่มี 131/220) · ยังไม่ได้ตกลงว่า `black IS NULL` (1 ราย) นับเป็นติดหรือไม่ติด
...
**2 ช่องที่ `uprice` ไม่มีคอลัมน์รองรับ — ถอดออกจากฟอร์มแล้ว 07/09/2569** (ผู้ใช้สั่ง; เดิมช่องยังอยู่แต่แปะ `wip` และไม่เคยถูกบันทึก):

| ช่องเดิมบนฟอร์ม | คอลัมน์ที่เคยใช้ | สถานะตอนนี้ |
|---|---|---|
| วันที่แจ้งปรับ | `tb_saleinfo.NotifyDate` | **ลบทั้งช่องกรอกและคอลัมน์ในตารางประวัติ** |
| MOQ (kg) | `tb_saleinfo.MOQ` | เหมือนกัน |

- ถอดออกจาก `SaleinfoController::COLUMNS` และ `extractForm()` อยู่แล้ว (คอมเมนต์ค้างไว้ทั้งคู่) — **ถ้าใส่กลับจะกลายเป็นคอลัมน์ที่ไม่มีจริงตอน insert/update = SQL error**
- 07/09/2569 ตามมาด้วย: ลบ 2 `<div>` ในกลุ่ม "ราคา" ของ `modal-price.blade.php` (ช่อง `DATE`/`PRICE` ที่เหลือ **คง `col-md-3` ตามเดิม** — เคยขยายเป็น `col-md-6` แล้วช่องกรอกยาวเกิน) · **ถอด `align-items-end` ออกจาก `row`** เพราะ label ช่องราคายาว 2 บรรทัดได้ ทำให้ช่องวันที่ถูกดันชิดล่างไม่ตรงหัวแถว · ลบ 2 `<th>` + 2 `<td>` ของตารางประวัติ (เหลือ 4 คอลัมน์: วันที่เริ่มซื้อ · รหัสสินค้า · ราคา · หมายเหตุ) · ตัด `NotifyDate`/`MOQ` ออกจาก loop เติมฟอร์มใน `index.blade.php` · `edit()` และ `history()` **เลิกส่งค่าว่าง 2 key นี้** ให้ฝั่งจอ
- `history()` ยังเรียง `orderByRaw('COALESCE(\`DATE\`, AuthDate) DESC')` — **ห้ามอ้าง `NotifyDate`** จะได้ SQL error
- **ถ้าจะเอากลับ:** เพิ่ม 2 คอลัมน์เข้า `uprice` ก่อน → คืนชื่อเข้า `COLUMNS` + ปลดคอมเมนต์ใน `extractForm()` + สร้างช่องในฟอร์มใหม่

⚠ **ช่อง `DATE` เปลี่ยนป้ายบนฟอร์มเป็น "วันที่เริ่มซื้อ" แล้ว (07/09/2569)** — ทั้งช่องกรอกและหัวคอลัมน์ในตารางประวัติของ `modal-price.blade.php` · **คอลัมน์ใน DB ยังชื่อ `DATE` เหมือนเดิม** · หน้ารายการ (`saleinfo/table.blade.php` หัวตาราง, ตัวกรอง `date_from`/`date_to` และ `FILTER_LABELS` ใน `index.blade.php`) **ยังใช้ป้ายเดิม "วันที่เริ่มราคาใหม่"** — ผู้ใช้สั่งเฉพาะในฟอร์ม

⚠ **`uprice.CustNo` เป็น varchar(5) — แคบกว่า `tb_saleinfo` เดิมที่เป็น varchar(6)** (`customer.code` ยาวสุด 5 ตัวพอดี ไม่มีรายไหนเกิน) ⇒ ใส่ `maxlength` ให้ช่องในฟอร์มตามความกว้างจริงแล้ว: CustNo 5 · st_code/ITEMNO 17 · REM1/REM2 100 · PackRem/Label 85 · Author 50 — กัน **error 1406 Data too long** (MySQL เปิด `STRICT_TRANS_TABLES`)

**ช่องหมายเหตุมี 2 ช่อง = `uprice.REM1` / `REM2` (เปิด REM2 คืน 07/09/2569 ตามที่ผู้ใช้สั่ง)** — ป้ายบนจอคือ "หมายเหตุ 1" / "หมายเหตุ 2" (`col-12` ทั้งคู่, varchar(100) เท่ากัน) · เดิม REM2 ถูกคอมเมนต์ปิดไว้ทั้งในฟอร์มและ `SaleinfoController::COLUMNS` เพราะเข้าใจว่าเลิกใช้แล้ว (มีตาราง "ประวัติการปรับราคา" แทน) แต่ **ข้อมูลจริงมี REM2 อยู่ 23,281 จาก 60,603 แถว (38%)** และหน้าอนุมัติใบสั่งซื้อก็อ่าน `uprice.REM2` มาโชว์อยู่แล้ว · ทดสอบ insert/update ผ่าน controller แล้วว่าค่าลง DB จริง (ลบแถวทดสอบทิ้งแล้ว) · ⚠ ตาราง "ประวัติการปรับราคา" ในฟอร์มยังโชว์เฉพาะ `REM1` — `history()` ไม่ได้คืน `REM2`
...
**ที่เอาไปใช้แล้ว (01/09/2569):** ช่อง **"อนุมัติราคาถึง"** ในฟอร์มขออนุมัติราคาพิเศษ ใช้ `App\Services\HolidayService::nextWorkingDay()` หา "วันทำการถัดไป" (ดูหัวข้อฟอร์ม MK ขออนุมัติราคาพิเศษ)

**เตือนเมื่อเลือกวันตรงกับวันหยุด ในฟอร์มแก้ไข Planning Item + modal เพิ่ม Semi (03/09/2569):** เมื่อผู้ใช้เลือก/เปลี่ยนวันแล้วตรงกับ **วันอาทิตย์** (`config/holiday.php` → `weekly_off`) หรือ **`tb_holiday` ที่ `is_active='Y'`** จะเด้ง Swal ยืนยัน; กด **"ยกเลิก" = ย้อนวันกลับเป็นค่าก่อนหน้า**. ช่องที่ครอบ:
  - **ฟอร์มแก้ไข Planning Item หลัก** (`planning-item-form.blade.php`) — วันที่สั่ง (`mdate`) · วันที่ต้องการรับ (`custwant`) · วันที่กำหนดทบทวน (`senddate`) · วันที่วางแผนผลิต (`inplan`)
  - **modal เพิ่ม Semi** (`#sp_entry_modal`, ช่องจาก partial `semi-pigment/partials/entry-fields` prefix `sp`) — วันที่สั่ง (`#sp_mdate`, Order Date) · วันที่ต้องการรับ (`#sp_custwant`, Want Date). modal นี้ถูก `appendTo('body')` จึงอยู่นอก `#planning_item_form` → ผูกแยก
  - **modal "สร้างแผน (Semi)"** (`#createSemiModal`, prefix `cs`, อยู่บนหน้า `index.blade.php`) — วันที่สั่ง (`#cs_mdate`) · วันที่ต้องการรับ (`#cs_custwant`). เปิดได้โดยไม่ต้องโหลดฟอร์มแก้ไข Item ก่อน → **นิยาม helper + ฝังข้อมูลวันหยุดที่หน้า index เอง** (guard `if (typeof window.wpBindHolidayWarn !== 'function')` กันนิยามซ้ำกับตัวใน partial), ผูกใน `$(document).ready` หลัง init flatpickr ของ createSemiModal
  - **ตรรกะกลาง = `window.wpBindHolidayWarn(el, label)`** (นิยาม 2 ที่ให้เหมือนกันเป๊ะ: บล็อก helper ก่อน IIFE ใน `planning-item-form.blade.php` และหน้า `index.blade.php` — ตัวไหนโหลดก่อนก็ทำงาน) — เก็บ `data-last` = ค่าล่าสุดที่ยืนยันแล้ว, ผูก event `change.wpholiday` บน input ตัวจริง (flatpickr ยิง change เมื่อเลือก/พิมพ์วัน), **revert แบบ self-contained ผ่าน flatpickr ของช่องเอง** (ไม่พึ่ง `wpSetDateField` เพราะหน้า index ไม่มี). `wpHolidayReason(ymd)` คืนชื่อวันหยุด/วันในสัปดาห์ (null = วันทำการปกติ). **`wpSetDateField()` ใน partial ถูกปรับให้ sync `data-last` ทุกครั้งที่ตั้งค่าด้วยโปรแกรม** (เช่น `fillModal` โหมดแก้ไข Semi) เพื่อให้ย้อนกลับตอนกดยกเลิกได้ค่าที่ถูก
  - **ตรวจฝั่ง client ล้วน** — `ProductionPlanController::editItem()` (ฟอร์ม Item + modal เพิ่ม Semi) และ `index()` (modal สร้างแผน) ส่ง `holidays` = `HolidayService::activeMap()` (`{'Y-m-d': 'ชื่อวันหยุด'}`) + `weekly_off` = `HolidayService::weeklyOff()` → JS เก็บใน `window.WP_HOLIDAY_MAP`/`WP_WEEKLY_OFF` แล้วเช็คสด ไม่มี round-trip (ยังไม่มีด่านฝั่ง server — เป็นแค่การเตือน ไม่บล็อกบันทึก)
  - แยกจากการยืนยัน custwant ตอน**กดบันทึก** (`data-original` ใน `index.blade.php`) ที่มีอยู่เดิม — คนละจังหวะ คนละเรื่อง (อันนั้นเตือนว่า custwant เปลี่ยน, อันนี้เตือนว่าตรงวันหยุด)
  - `HolidayService::activeMap()` = helper ใหม่คืน map พร้อมชื่อวันหยุด (refactor `holidayMap()` ให้เก็บ `'Y-m-d' => 'ชื่อ'` แทน `=> true`; `isHoliday()` ยังใช้ `isset()` จึงไม่กระทบ)
  - **ที่อื่นยังไม่ผูก** (เตือนช่องวันที่ในหน้าอื่น ฯลฯ) จะเพิ่มก็เรียก `window.wpBindHolidayWarn` / `HolidayService` ตัวเดิม
...
## จัดการเครื่องจักร (เมนู "จัดการเครื่องจักร", `/production-planning/machine`)

`MachineController` (อยู่ที่ `app/Http/Controllers/MachineController.php` — **ไม่ใช่** โฟลเดอร์ `Production/`) + route ใน `routes/production.php` (`machine.index` / `machine.datatable` / `machine.edit` / `machine.store` / `machine.delete`) + view `machine/{index,machine-form}.blade.php` + Model `Machine` (`machine`, MyISAM legacy, `$timestamps = false`) — CRUD เครื่องจักรรายแผนก

**คอลัมน์ตาราง `machine`:** `id` · `dept` varchar(50) · `MBX` varchar(20) (ชื่อ/รหัสเครื่อง) · `speed_rpm` varchar(255) · **`group` varchar(50)** (ประเภท/กลุ่ม — เพิ่ม 02/09/2569)

- **คอลัมน์ `group` (02/09/2569):** ประเภท/กลุ่มของเครื่องจักร ข้อความสูงสุด 50 ตัว — เพิ่มด้วย migration `2026_09_02_100000_add_group_to_machine_table` (idempotent). แสดง+แก้ไขในฟอร์ม modal (`machine-form.blade.php`, `name="group"` maxlength 50) · บันทึกผ่าน `store()` (validate `nullable|string|max:50`) · โชว์เป็นคอลัมน์ในตารางรายการ (sort ได้) · ค้นหาได้ 2 ทาง: **ช่องค้นหาข้อความ** (LIKE รวมกับ `MBX`/`dept`) และ **dropdown `#searchGroup`** (`request('group')` = exact match, ตัวเลือกจาก `groupCodes()` = ค่า group ที่มีจริงในตาราง)
  - ⚠ **`group` เป็น reserved word ของ MySQL** — โค้ดทั้งหมดผ่าน Eloquent จึง backtick อัตโนมัติ (select/orderBy/where/updateOrCreate ปลอดภัย) แต่ถ้าเขียน **raw SQL** อ้างคอลัมน์นี้ต้องใส่ `` `group` `` เอง
...
## จัดการกลุ่มราคา (เมนู "จัดการกลุ่มราคา", `/color-rate`) — 19/09/2569

`ColorRateController` + Model `ColorRate` (**`zcolorrate`**) + `routes/colorrate.php` + view `color-rate/{index,color-rate-form,lookup-modal,lookup-table}.blade.php` — CRUD **ราคาขั้นต่ำของกลุ่ม A/B/C แยกตามรหัสสินค้า**

**ทำไมต้องมี:** ค่าใน `zcolorrate` ถูกใช้เป็น **เกณฑ์ขั้นต่ำของด่านราคา** ตอนบันทึกใบสั่งซื้อ (`OrderController::colorRateFloor()`) และเป็นช่อง **"ขั้นต่ำ"** ในกล่องราคา — แต่เดิม**ไม่มีหน้าจอเลย** ต้องแก้ใน DB ตรง ๆ

| คอลัมน์ | เก็บอะไร |
|---|---|
| `colorno` | รหัสสินค้า varchar(12) — **PK (ไม่มี `id`)** |
| `rate_A` / `rate_B` / `rate_C` | ราคาขั้นต่ำของกลุ่ม A / B / C (float) |
| `RDate` | วันที่ปรับราคา (datetime — ข้อมูลเดิมเป็น 00:00:00 ทุกแถว) |

- **ตาราง legacy MyISAM · 28 แถว · ไม่มี `id` / ไม่มี timestamps** ⇒ Model ตั้ง `$primaryKey='colorno'`, `$incrementing=false`, `$keyType='string'`, `$timestamps=false`
- 🔴 **ตอนแก้ไข รหัสสินค้าเป็น `readonly`** — เป็น PK ถ้าพิมพ์ผิดต้องลบแล้วเพิ่มใหม่ · `store()` ยึด `colorno` เดิม (ส่งมาในช่อง `id`) เสมอ ไม่สนค่าที่ส่งมาในช่อง `colorno`
- **เพิ่มใหม่กันรหัสซ้ำเอง** (`->exists()` ก่อน insert) ไม่งั้นชน PK แล้วได้ SQL error ดิบ
- ⚠ **ต้องถอดคอมมา *ก่อน* validate** — ช่องราคาใช้ `js-comma` ⇒ ค่าที่ส่งมาคือ `1,234.50` ซึ่งกฎ `numeric` **ตีกลับ** (ฝั่งจอเรียก `stripCommaFields()` ให้แล้วก็จริง แต่ยิงตรง/JS ไม่ทำงานจะเจอ) → `store()` `$request->merge()` ถอดคอมมาก่อนเข้า Validator
- **ไม่กรอก `RDate` = วันนี้ 00:00:00** (`now()->startOfDay()` ให้ตรงกับข้อมูลเดิม)
- โครง controller/blade มิเรอร์จาก **QcStatusController** (DataTables serverSide + modal ฟอร์ม) แต่ **ไม่มี `is_active`** (ตารางไม่มีคอลัมน์นี้) จึงไม่มี switch สลับสถานะ
- **เกณฑ์น้ำหนักของกลุ่ม A/B/C ยังอยู่ที่ `PriceApprovalController::PRICE_GROUPS` เหมือนเดิม** (hard-code 1,000 / 500 / 0) — เมนูนี้จัดการเฉพาะ *ราคา* ไม่ได้จัดการ *เกณฑ์* · เพิ่ม **`PriceApprovalController::priceGroups()`** (public static) ให้หน้าอื่นอ่านป้ายกลุ่มได้โดยไม่ต้อง hard-code ซ้ำ

**ปุ่ม "ดูกลุ่มราคา" (อ่านอย่างเดียว) ในหน้า `/saleinfo` และ `/order`:** partial ร่วม **`color-rate/lookup-modal.blade.php`** (`@include` ที่ท้าย content ของทั้ง 2 หน้า) — modal ตารางค้นหารหัสได้ + แถบบอกเกณฑ์กลุ่ม เปิดด้วย `onclick="openColorRateLookup()"`
- ข้อมูลมาจาก `GET color-rate/lookup` → คืน **HTML** (partial `lookup-table`) ให้ยัดลง modal ได้เลย
- 🔴 **route name เป็น `colorratelookup.data` ไม่ใช่ `colorrate.*` โดยตั้งใจ** — `CheckAccess` คุมสิทธิ์ด้วย **namespace ของ route name เทียบกับเมนู** ถ้าใช้ `colorrate.*` คนที่เข้าเมนู Order / กำหนดราคา ได้ แต่ไม่ได้ติ๊กเมนู "จัดการกลุ่มราคา" จะกดปุ่มนี้แล้ว **403** · namespace ที่ไม่ผูกเมนูจะ pass-through (เหลือด่าน `auth`)
- **JS อยู่ในตัว partial** (guard `if (typeof window.openColorRateLookup !== 'function')` กันนิยามซ้ำ) — ตัวฟังก์ชันเรียก jQuery **ตอนถูกกดเท่านั้น** จึงนิยามไว้ก่อน `inc_js` ได้ · การผูก event ใช้ `DOMContentLoaded` ไม่ใช่ `$(function(){})` เพราะตอน parse ถึงตรงนั้น jQuery ยังไม่โหลด
- 🔴 **ห้ามพิมพ์ชื่อ Blade directive (เช่น at-section) ในคอมเมนต์ JS ของ partial** — Blade ตีความเป็น directive จริง แล้ว**กลืน HTML ที่เหลือทั้งหน้า** (เจอจริงตอนทำ 19/09/2569: หน้า render ออกมาเหลือ 411 bytes)
- ⚠ **ความเสี่ยงแบบเดียวกับเมนูอื่น:** route `colorrate/store` · `colorrate/delete` ป้องกันแค่ `auth` + สิทธิ์ระดับเมนู — ไม่มีสิทธิ์เขียน/ลบแยก และ **ไม่มี audit trail** (ตารางไม่มีคอลัมน์ผู้แก้ไข — `RDate` เป็นวันที่ปรับราคา ไม่ใช่เวลาที่แก้เรคคอร์ด)
- ⚠ **การลบกระทบด่านราคาทันที** — รหัสที่ถูกลบจะตกไปใช้ "ราคาช่อง 2" เป็นเกณฑ์แทน (ดูตารางลำดับเกณฑ์ในหัวข้อด่านราคา)

## สถานะ QC (เมนู "สถานะ QC", `/production-planning/qc-status`) — 09/09/2569

`QcStatusController` + Model `QcStatus` (`tb_qc_status`, InnoDB, timestamps เปิด) + view `qc-status/{index,qc-status-form}.blade.php` — ตาราง master ของ**สถานะ QC สำเร็จรูป** (CRUD)

- **route อยู่ใน `routes/production.php`** (ใต้ prefix `production-planning`) แต่ **route name เป็น namespace แบน `qcstatus.*`** (`index`/`datatable`/`edit`/`store`/`delete`/`toggle-status`) — ไม่ใช่ `production.*` (มิเรอร์ pattern จาก `planningremark.*`)
- **เมนู `QcStatus`** (title "สถานะ QC") วาง**ใต้ `PlanningStatus` (สถานะ Planning)** ใน `config/menu.php` — สิทธิ์คุมด้วย **menu key** (`AccessControl` เทียบ namespace ของ route name = `qcstatus`) → พนักงาน (guard `emp`) ต้องไปติ๊กเมนูนี้ให้ role ที่หน้า "จัดการสิทธิ์" ก่อน (admin เข้าได้อยู่แล้ว)
- โครง controller/blade มิเรอร์จาก **`PlanningRemarkController`** ทุกอย่าง (หน้ารายการ DataTables serverSide + modal ฟอร์ม + สลับสถานะจากตาราง + ช่อง `sort` เรียงเริ่มต้น `sort`→`id`) **ยกเว้น: ตาราง `tb_qc_status` ไม่มีคอลัมน์ `created_by`/`updated_by`** จึง `store()`/`toggleStatus()` ไม่เขียน 2 คอลัมน์นี้ (ไม่มี audit trail เลย)
- คอลัมน์: `id` · `name` varchar(255) nullable · `sort` int nullable · `is_active` enum(`Y`/`N`) default `Y` · `created_at`/`updated_at` — **ตารางมีอยู่ใน DB ก่อนแล้ว (สร้างมือ)**; migration `2026_09_09_100000_create_tb_qc_status_table` (idempotent, `Schema::hasTable`) ไว้สร้างให้ server ที่ยังไม่มี — บนเครื่องที่มีอยู่แล้วเป็น no-op
- **ข้อมูลตั้งต้น 3 แถว (ผ่าน/ไม่ผ่าน/รอสูตรปรับแก้) อยู่ที่ `database/seeders/QcStatusSeeder.php`** (ลงทะเบียนใน `DatabaseSeeder` แล้ว) — ใช้ `firstOrCreate` คีย์ด้วย `name` ⇒ **รันซ้ำได้ ไม่ทับค่าที่ผู้ใช้แก้เอง** (`sort`/`is_active` เป็นค่าเริ่มต้นตอนสร้างครั้งแรกเท่านั้น)
- ⚠ **ความเสี่ยงแบบเดียวกับเมนูอื่น:** route ป้องกันแค่ `auth` + สิทธิ์ระดับเมนู — ไม่มีสิทธิ์เขียน/ลบแยก และไม่มี audit trail

**นำไปใช้เป็น dropdown ในฟอร์มแก้ไข Planning Item แล้ว (09/09/2569):** ช่อง "สถานะ Qc (QC Status)" ใน `planning-item-form.blade.php` เดิมเป็น select ที่ **hard-code 3 ค่า** (`PASSED`/`FAILED`/`PENDINGREVISION`) → เปลี่ยนเป็น select ที่ **ดึงตัวเลือกจาก master `tb_qc_status` (`is_active='Y'`)**
- 🔵 **เก็บเป็น "id" (FK → `tb_qc_status`) ไม่ใช่ข้อความ** (ผู้ใช้เลือก 09/09/2569 — ต่างจาก [[planning_remark]] ที่เก็บข้อความ snapshot): บันทึก **id ของสถานะลง `tb_planning.qc_status`**
  - **คอลัมน์ `tb_planning.qc_status` เปลี่ยนจาก `ENUM('PASSED','FAILED','PENDINGREVISION')` → `BIGINT UNSIGNED NULL`** ด้วย migration `2026_09_09_110000_change_qc_status_to_id_on_tb_planning_table` (idempotent เช็คชนิดคอลัมน์; แปลง enum→varchar→map ค่าเดิม→bigint). **ค่าเดิม 2 แถว `PASSED` ถูก map เป็น id ของ "ผ่าน"** (map: PASSED→ผ่าน · FAILED→ไม่ผ่าน · PENDINGREVISION→รอสูตรปรับแก้ — migration ทำ `firstOrCreate` master กันกรณี seeder ยังไม่รัน). `down()` แปลงกลับเป็น enum ได้ (map id→สตริง; ค่าอื่นที่ไม่ตรง 3 ค่า → NULL)
  - **`saveItem`:** validation เปลี่ยนเป็น `nullable|integer|exists:tb_qc_status,id` + normalize `qc_status` ('' → `null`, ไม่งั้น '' ลง BIGINT column พัง) ก่อน create/update
  - **รายงานการขาดวัตถุดิบ (`buildMaterialShortageReport`) `LEFT JOIN tb_qc_status`** แล้ว select `tb_qc_status.name as qc_status` → ค่าที่ออกมายังเป็น "ชื่อสถานะ" (ไม่ใช่ id) คอลัมน์แสดงผล QC ในรายงาน (Excel/PDF) ที่ **คอมเมนต์ปิดไว้** เปิดคืนได้เลยไม่ต้องแก้เพิ่ม
  - Model `Planning::qcStatus()` = `belongsTo(QcStatus::class, 'qc_status')` (ใช้ `optional($p->qcStatus)->name` ได้)
- **คงค่าเดิมที่อ้าง id ของสถานะที่ถูกปิด/ลบไว้** — `editItem()` ถ้า id ที่บันทึกไว้ไม่อยู่ในรายการ active จะ `push` แถวนั้นกลับเข้า `$qc_statuses` ให้ยังแสดง (กันค่าหายเมื่อ master ถูก deactivate ทีหลัง)
- **controller:** `ProductionPlanController::editItem()` ส่ง `$qc_statuses` = `QcStatus::where('is_active','Y')->orderBy('sort')->orderBy('id')->get(['id','name'])` (+ แถว current ที่ inactive)
- ⚠ **เป็น plain `<select>` ไม่ใช่ `select2-tags`** (ต่างจาก planning_remark) — QC status เป็นรายการควบคุม (พิมพ์ค่าเองไม่ได้) จึงไม่ต้อง init select2. master มีข้อมูลตั้งต้น 3 แถวจาก `QcStatusSeeder` (ผ่าน/ไม่ผ่าน/รอสูตรปรับแก้) — เพิ่ม/แก้ได้ที่เมนู "สถานะ QC"
...
- **สถานะ "กำลังโหลด" ของตารางรายการ — `showTableLoading()` / `showTableError()`** (`resources/views/layout/inc_js.blade.php`, 16/09/2569): server ของลูกค้าดึงข้อมูลช้า เดิมกดค้นหา/กรองแล้ว**ตารางชุดเก่าค้างอยู่เงียบ ๆ** จนข้อมูลใหม่มา (spinner "กำลังโหลดข้อมูล..." มีแค่ตอนเปิดหน้าครั้งแรก เป็น HTML ที่วางไว้ใน `#table-data` รอ AJAX มาแทน) ⇒ ผู้ใช้ไม่รู้ว่ากดติดไหมแล้วกดซ้ำ
  - ตัวช่วยกลาง 2 ตัวใน `inc_js` (ใช้ได้ทุกหน้าไม่ต้อง include เพิ่ม) — `showTableLoading(scope)` แทนที่กล่องตารางด้วย spinner + "กำลังโหลดข้อมูล..." · `showTableError(scope, message)` แสดงข้อความแดง "โหลดข้อมูลไม่สำเร็จ — กรุณาลองใหม่อีกครั้ง" · `scope` ไม่ระบุ = `#table-data` (id ที่หน้ารายการทุกหน้าใช้ตรงกัน)
  - **วิธีผูกใน `loadData()`**: เรียก `showTableLoading();` หลัง `dtXhr.abort()` ก่อนยิง ajax + เพิ่ม `error` handler เรียก `showTableError()`
  - 🔴 **ต้องมี error handler คู่กันเสมอ** — ไม่งั้น request ที่พังจะทิ้ง spinner ค้างถาวร (แย่กว่าเดิมที่ตารางชุดเก่ายังอยู่) · 🔴 **ใน error handler ต้องข้าม `textStatus === 'abort'`** (พิมพ์เร็ว ๆ แล้ว `dtXhr.abort()` ตัด request เก่าทิ้งเป็นเรื่องปกติ ไม่ใช่ error) คู่กับ guard `seq !== dtSeq` เดิม
  - ⚠ **`@include('layout/inc_js')` ต้องมาก่อน `<script>` ของหน้า** — หน้ารายการเรียก `loadData(page)` ทันทีตอนโหลดสคริปต์ ถ้า helper ยังไม่ถูกนิยามจะ throw แล้วตารางไม่โหลดเลย (ตรวจแล้วทั้ง 5 หน้าเรียงถูกอยู่แล้ว)
  - ผูกแล้ว 5 หน้า: **Order · ฐานข้อมูลลูกค้า · ใบเสนอราคา · เทียบสี · กำหนดราคา** (หน้าอื่นที่มี `loadData()` — production-planning / permission / user / equipments / category / position / equiptrack — **ยังไม่ผูก** เรียก 2 ฟังก์ชันนี้เพิ่มได้เลย)
...
- ⚠ **ตาราง legacy เป็น MyISAM ⇒ `DB::transaction()` และ `lockForUpdate()` ไม่ทำงาน** (ตรวจพบ 21/08/2569): ฐานข้อมูลมี **InnoDB 25 ตาราง / MyISAM 33 ตาราง** — ตารางของระบบเดิมที่ใช้บ่อยเป็น MyISAM ทั้งหมด (`morder`, `suborder`, `orderrun`, `appvreq`, `zcustprice`, …) ส่วนตาราง `tb_*` ที่สร้างใหม่เป็น InnoDB
  ...
  - 🔴 **ยิง `OrderController::save()` (และ `save()` ตัวอื่นที่เขียนทั้งแถว) ด้วย payload ไม่ครบ = ช่องที่ไม่ได้ส่งถูกเขียนเป็น `null` ทันที และย้อนไม่ได้** — `headerPayload()`/`syncItems()` ประกอบค่าจาก request **ทุกคอลัมน์** ไม่ได้ merge กับของเดิม (เจอจริง 21/09/2569: ทดสอบใบ `CI43205` แล้ว `Company`/`Emp`/`supno` กับ `suborder.senddate`/`custwant`/`nold` หาย — ซ่อมคืนจาก `public/sql/*.sql` แล้ว) ⇒ **ทดสอบ save() ให้สร้างใบทิ้งของตัวเอง หรือจดทั้งแถว (`SELECT *`) ไว้ก่อนเสมอ**
...
- **ตารางเงื่อนไขคิดราคาขาย** (`config/product_price.php` + `ProductPriceService`, 10/08/2569): แต่ละแถวมี `key` (ห้ามเปลี่ยน/ซ้ำ), `prefix`, `suffix`, `suffix_pos` (= ตัวลงท้ายต้องเริ่มที่ตัวที่ N พอดี เช่น "MB ตัวที่ 8 ลงท้ายด้วย P/PC/K/J") และ `mul`/`div`/`add`
  ...
  - **จอ Test Price (`/saleinfo` → modal `saleinfo/modal-testprice.blade.php`) ต่อข้อมูลแล้ว (25/08/2569)** — อ่านอย่างเดียว ไม่มีบันทึก ผ่าน `GET saleinfo/test-price` → `SaleinfoController::testPrice()`
    ...
    - **ราคา 1/2/3 + DB 3-4 / 1-2 Kg. = `ProductPriceService::quote(เบอร์ที่ตั้ง, TNet)`** — เมธอด `quote()` เพิ่มใหม่ (25/08/2569) = สูตรเดียวกับ `lookup()` ทุกอย่าง ต่างแค่ **รับราคาทุนมาเอง** แทนการอ่าน `access_pdprice` (`lookup()` ถูก refactor ให้เรียก `quote()` ต่อ) เพราะเบอร์ที่เพิ่งตั้งจากใบเทสมักยังไม่มีในตารางราคาทุน
      - **ทั้ง 5 ช่อง (Price 1/2/3 + DB 3-4 / 1-2 Kg.) โชว์เป็นจำนวนเต็ม** ตั้งแต่ 07/09/2569 — `commaFmt(v, 0)` ชุดเดียวกับฟอร์ม "กำหนดราคา" (`setCalcField`) และจอ "ค้นหาราคาสินค้า" (`npMoneyInt`) · ปัดตอนแสดงผลอย่างเดียว ค่าจาก endpoint ยังเป็นทศนิยม
      - ⚠ **ตัวเลขไม่จำเป็นต้องตรงกับฟอร์ม "กำหนดราคา" ของเบอร์เดียวกัน** — สูตร/เงื่อนไขเดียวกันเป๊ะ แต่ **ราคาทุนคนละที่มา**: Test Price ใช้ `access_testmai.TNet` (ต้นทุนสูตรของใบเทส) ส่วนฟอร์มกำหนดราคาใช้ `access_pdprice` เช่น เบอร์ `221Y572MB` = 1,182.15 กับ 1,185 ⇒ ราคาต่างกันเล็กน้อยตามต้นทุน (ตั้งใจไว้แบบนี้)
    ...
    - 🔴 **ตรวจ 07/09/2569: คิดราคาได้จริงแค่ 19 จาก 2,460 ใบ — เป็นเรื่องข้อมูล ไม่ใช่บั๊ก** · `access_compo` (ตัวที่บอกว่าใบเทสตั้งเป็นเบอร์อะไร) มีแค่ **130 TestNo / 758 แถว** และในนั้นตรงกับใบใน `access_testmai` เพียง **20 ใบ** (อีก 110 ใบเป็นใบเก่าที่ไม่มีใน testmai) ⇒ **2,440 จาก 2,460 ใบ (99.2%) ขึ้น "ใบเทสนี้ยังไม่ได้ตั้งเบอร์"** รวมใบล่าสุดทั้งหมดของปี 2569 · สาเหตุ: ไฟล์อัพเดท `formula_2000_AddData.mdb` (27/08/2569) มีแค่ `P_PdPrice` / `P_TestMai` **ไม่มีตาราง Compo** ⇒ ต้องขอไฟล์ที่มี Compo จากลูกค้าเพิ่ม แล้วนำเข้าด้วย OLEDB (ห้ามใช้ ODBC ของ PHP — ดูหัวข้อภาษาไทยจากไฟล์ Access) · ไม่มีใบไหน fail เพราะจับเงื่อนไขราคาไม่ได้เลย (`no_rule` / `rule_zero` = 0)
    - ⚠ **ยังไม่ได้กันเคส `TNet` ≤ 0** — `quote()` คิดราคาจากต้นทุน 0 ให้เลย ได้ผลเป็นค่าคงที่จากตัว `add` ของเงื่อนไข (เช่น Lot `690605/52` เบอร์ `BL7A032CP` TNet 0 → โชว์ 3 / 3 / 4 / 5 / 6) ⇒ ราคาที่ไม่มีความหมายแต่ดูเหมือนใช้ได้ · ถ้าจะกัน ให้เช็คที่ `SaleinfoController::testPrice()` ก่อนเรียก `quote()` แล้วคืน reason แบบเดียวกับ "ยังไม่ได้ตั้งเบอร์" (แก้ใน `quote()` จะกระทบ `lookup()` ที่หน้าอื่นใช้ด้วย)
