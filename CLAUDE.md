...
**ต้องกรอกรหัสผ่านก่อนอนุมัติ (12/09/2569 ตามที่ผู้ใช้สั่ง):** กดติ๊ก "อนุมัติ" → Swal เดิม (ยืนยันพร้อมข้อมูลใบ) **เพิ่มช่อง `input: 'password'`** → ส่ง `password` ไปกับ `POST order/order-approval/approve` → `OrderApprovalController::approvePasswordError()` ตรวจ **ก่อนแตะ DB** ผิด/ไม่ส่ง = **422 + `bad_password: true`**
- 🔴 **รหัสผ่าน = รหัสผ่านของบัญชี "ใครก็ได้" ในระบบ** (ผู้ใช้สั่ง 16/09/2569 — เดิม 12/09/2569 ตรวจกับ**บัญชีที่ล็อกอินอยู่**เท่านั้น ผ่าน `AccessControl::currentAccount()->getAuthPassword()`; ก่อนหน้านั้นเคยเป็นรหัสร่วม `config('order.md_password')` แบบฟอร์ม MK): `approvePasswordError()` ดึง hash ทั้งหมดจาก **`emp.password` + `users.password`** (ตัดค่าว่าง/NULL) แล้ววน `Hash::check` — ตรงกับคนใดคนหนึ่ง = ผ่าน ⇒ **ไม่ผูกกับคนที่กด** (ไม่ได้ใช้ `AccessControl` แล้ว ถอด import ออก)
- ข้อความ 422 เหลือ 2 กรณี: **กรุณากรอกรหัสผ่าน** (ไม่ส่งค่ามา) · **รหัสผ่านไม่ถูกต้อง** (ไม่ตรงกับใครเลย) — เดิมมีกรณี "ไม่พบบัญชีที่ล็อกอิน" / "บัญชีนี้ยังไม่ได้ตั้งรหัสผ่าน" ซึ่งไม่เกี่ยวแล้ว · ⚠ `emp` มี 2 คอลัมน์รหัสผ่าน — ใช้ `password` (hash) ไม่ใช่ `pwd` ของระบบเก่า (varchar 10) · **มีพนักงานตั้งรหัสไว้แค่ 5 จาก 33 คน** + `users` 14 คน = **19 hash**
- ⚠ **bcrypt เทียบเป็นชุดไม่ได้ ต้องวนทีละ hash** — วัดจริงบนเครื่อง dev: รหัสถูก ~72 ms (เจอที่ `emp` ตัวแรก) ถึง ~450 ms · **รหัสผิด ~1.06 วินาที** (ต้องวนครบ 19 ตัว) ⇒ ถ้าบัญชีในระบบเพิ่มมาก เวลาจะโตตามจำนวน hash
- ⚠ **ด่านนี้อ่อนกว่าเดิมโดยเจตนา** — ใครมีรหัสของพนักงาน/แอดมินคนไหนก็กดอนุมัติได้ และระบบ**ไม่รู้ว่ารหัสที่กรอกเป็นของใคร** (ตรวจแค่ "ตรงกับใครสักคน") ⇒ ยังไม่มี audit trail เหมือนเดิม
- ⚠ **ไม่มี session ปลดล็อกแบบฟอร์ม MK** — ต้องกรอกทุกครั้งที่กด (ตรงตามที่ผู้ใช้สั่ง "เมื่อกดอนุมัติ ให้ป้อนรหัสผ่านก่อน")
...
⚠ **ความเสี่ยงที่ยังค้างอยู่** (เหมือน `POST order/save`): route นี้ป้องกันแค่ `auth` + สิทธิ์ `order read` จาก `config/menu.php` — **ไม่มีสิทธิ์แยกสำหรับการอนุมัติ** (รหัสผ่านด้านบนเป็นด่านเดียว ซึ่งเป็นรหัสร่วมที่ใครรู้ก็กดได้) และ **ไม่มี audit trail ว่าใครกด** — `morder` ไม่มีคอลัมน์ผู้อนุมัติ
