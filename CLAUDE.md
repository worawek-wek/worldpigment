...
⚠ ปุ่มที่เหลือ (**ตรวจสอบเฉพาะร้าน**) ยังโหลดผลลงตารางล่างเหมือนเดิม — ส่วน endpoint JSON `/other-items` `/history` `/resin-history` + `HISTORY_COLS` / `RESIN_COLS` ยังอยู่ครบแต่ไม่มีคนเรียกแล้ว (มีคอมเมนต์กำกับไว้ เผื่อกลับมาใช้)
