    ...
    @foreach ($invoice as $key => $row)
        ...
        <table class="table-info">
            <tr>
                <td>
                    ...
                    {{ $row->room_for_rent->renter->prefix.' '.$row->room_for_rent->renter->name.' '.$row->room_for_rent->renter->surname }}<br>
                    {{ $renter->address.' '.$renter->fullThaiAddress() }}<br>
                    เลขประจำตัวผู้เสียภาษี {{ $row->room_for_rent->renter->id_card_number }}<br>
                    โทร {{ $row->room_for_rent->renter->phone }}
                </td>
                <td style="text-align: right;">
                    <strong>ต้นฉบับ (Original)</strong><br>
                    เลขที่(ID) {{ $row->invoice_number }}<br>
                    วันที่(Date) {{ date('d/m/Y',strtotime($row->created_at)) }}<br>
                    ห้อง(Room) {{ $row->room_for_rent->room->name }}<br>
                    พนักงาน(Staff) {{ $row->user->name }}<br>
                </td>
            </tr>
        </table>
        <div class="full-width">
            <table class="table">
                ...
                @foreach ($row->payment_list as $key2 => $item_payment_list)
                    @php
                        $pd_5px = "";
                        if ($loop->first){
                            $pd_5px = "pdt-5px";
                    ...
                    @endphp
                    <tr>
                        <td class="{{ $pd_5px }}"> {{ $key2+1 }} </td>
                        <td class="{{ $pd_5px }}"> {{ $item_payment_list->title }}
                        <td class="{{ $pd_5px }}"> {{ $item_payment_list->title }}@if(strpos($item_payment_list->title, 'Water rate') !== false){{ number_format($item_payment_list->unit) }} - {{ $row->previous_water_unit ?? 0 }} = {{ $item_payment_list->unit-$row->previous_water_unit }} ยูนิต)@endif
                        </td>
        ...
        <table class="total-table">
            <tr style="vertical-align: top;">
                <td style="font-size: large;">({{ $amount_thai[$row->id] }})</td>
                <td>จำนวนเงินรวมทั้งหมด <br>(Total amount)</td>
                <td style="font-size: large;">
                    {{ number_format($row->total_amount) }} บาท
                </td>
            </tr>
        </table>
        <table>
            <tr>
                <td width="150px" style="padding-top: 5px;">
                    <img src="/upload/qr-code/{{ $row->room_for_rent->room->floor->building->qr_code }}" alt="" width="70%" >
                </td>
