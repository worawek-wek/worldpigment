...
@section('content')
                    ...
                    <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            ...
                            <p class="text-muted mb-0">
                                รายการสถานะ QC สำเร็จรูปสำหรับการตรวจสอบคุณภาพ
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-striped table-hover">
                                <thead class="table-light">
                                    <tr>
                                        <th class="col-1">#</th>
                                        <th class="col-2">ลำดับ</th>
                                        <th class="col-5">สถานะ QC</th>
    ...
    <!-- Create/Edit QC Status Modal -->
    <div class="modal fade modalHeadDecor" id="qcStatusModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="ti ti-clipboard-check me-1"></i>สถานะ QC
                    </h5>
...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#dataTable').DataTable({
            ...
            columns: [
                // # เป็นเลขลำดับที่นับหลัง query จึง sort ไม่ได้ (renumber ตามผลลัพธ์ที่เรียงแล้ว)
                { 'className': "text-center", data: 'rownum', name: 'rownum', orderable: false },
                { 'className': "text-center", data: 'sort', name: 'sort', orderable: true, searchable: false },
    ...
    $(document).on('change', '.switch_status', function(){
        ...
        $.ajax({
            url: "{{ route('qcstatus.toggle-status') }}",
            method: "POST",
            dataType: 'json',
    ...
    $(document).on('click', '#btn_add', function(e){
        e.preventDefault();
        $.ajax({
            url: "{{ route('qcstatus.edit') }}",
            method: "GET",
            data: { id: null },
            success: function(response) {
                $('#result_detail').html(response.data);
                $('#qcStatusModal').modal('show');
            }
        });
    });
    ...
    $(document).on('click', '#btn_qcstatus_save', function(e) {
        e.preventDefault();
        var $btn = $(this);
        var formData = $('#qcstatus_master_form').serialize();

        $btn.prop('disabled', true);
        $.ajax({
            url: "{{ route('qcstatus.store') }}",
            method: "POST",
            dataType: 'json',
            data: formData,
            success: function(response) {
                if (response.status == 200) {
                    $('#qcStatusModal').modal('hide');
                    oTable.draw(false); // draw(false) = คงหน้า pagination เดิม (ไม่เด้งกลับหน้า 1)
                    Swal.fire({
                        icon: 'success',
                        title: 'สำเร็จ',
