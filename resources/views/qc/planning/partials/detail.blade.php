{{-- Modal ดูรายละเอียดงาน (อ่านอย่างเดียว) — QC --}}
...
<div class="modal-body">
    <div class="row g-3">
        ...
        <div class="col-md-4">
            <div class="small text-muted">ส่ง QC เมื่อ</div>
            <div class="fw-semibold">
                @if($job->qc_date)
                    {{ \Carbon\Carbon::parse($job->qc_date)->format('d/m/Y') }}{{ $job->qc_time ? ' ' . \Carbon\Carbon::parse($job->qc_time)->format('H:i') : '' }}
                @else
                    -
                @endif
            </div>
        ...
        <div class="col-md-4">
            <div class="small text-muted">ผล QC</div>
            <div class="fw-semibold">
        ...
        <div class="col-md-4">
            <div class="small text-muted">บันทึกผล QC เมื่อ</div>
            <div class="fw-semibold">{{ $job->qc_datetime ? \Carbon\Carbon::parse($job->qc_datetime)->format('d/m/Y H:i') : '-' }}</div>
        </div>
    </div>
