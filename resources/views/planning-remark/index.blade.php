...
@section('content')
                    ...
                    <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            <h3 class="mb-1">
                                ...
                                จัดการหมายเหตุวางแผน
                            </h3>
                            <p class="text-muted mb-0">
                                รายการหมายเหตุสำเร็จรูปสำหรับแผนการผลิต
                            </p>
                        </div>
                    </div>
                </div>
    ...
    <!-- Create/Edit Planning Remark Modal -->
    <div class="modal fade modalHeadDecor" id="remarkModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="ti ti-notes me-1"></i>หมายเหตุวางแผน
                    </h5>
...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#dataTable').DataTable({
            ...
            responsive: true,
            ajax: {
                url: "{{ route('planningremark.datatable') }}",
                data: function(d) {
    ...
    $(document).on('change', '.switch_status', function(){
        ...
        $.ajax({
            url: "{{ route('planningremark.toggle-status') }}",
            method: "POST",
            dataType: 'json',
    ...
    $(document).on('click', '.btn_delete', function(e){
        ...
        Swal.fire({
            ...
            $.ajax({
                url: "{{ route('planningremark.delete') }}",
                method: "POST",
                dataType: 'json',
    ...
    $(document).on('click', '#btn_remark_save', function(e) {
        e.preventDefault();
        var $btn = $(this);
        var formData = $('#remark_master_form').serialize();

        $btn.prop('disabled', true);
        $.ajax({
            url: "{{ route('planningremark.store') }}",
            method: "POST",
            dataType: 'json',
            data: formData,
            success: function(response) {
