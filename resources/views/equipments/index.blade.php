...
<body>
                                            ...
                                            <div class="col-sm-6 text-end">
                                                <button type="button" class="btn btn-main waves-effect waves-light"
                                                    data-bs-toggle="modal" data-bs-target="#addModal"><i
                                                        class="ti-xs ti ti-plus me-2"></i>เพิ่มอุปกรณ์</button>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="modal fade modalHeadDecor" id="addModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <form id="insert_equipments" enctype="multipart/form-data">
                @csrf
                <div class="modal-content rounded-0">
                    <div class="modal-header rounded-0">
                        <h5 class="modal-title" id="exampleModalLabel1">เพิ่มข้อมูลอุปกรณ์</h5>
    ...
    <div class="modal fade modalHeadDecor" id="history" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content rounded-0">
                <div class="modal-header rounded-0">
                    <h5 class="modal-title" id="exampleModalLabel1">ประวัติการเบิก</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                {{-- <div class="modal-body px-5" id="history-table">
                        ...
                        <table class="table table-bordered table-hover align-middle">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>-</td>
                                    <td>2</td>
                                    <td><span class="badge bg-warning">ใกล้ครบกำหนด</span></td>
                                    <td>-</td>
                                </tr>
                                <tr>
                                    <td>2</td>
                                ...
                                <tr>
                                    ...
                                    <td>1</td>
                                    <td><span class="badge bg-success">คืนแล้ว</span></td>
                                    <td><span class="badge bg-danger">ชำรุด</span></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    <div class="modal fade modalHeadDecor" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <form id="update_equipments" enctype="multipart/form-data">
                @csrf
                <div class="modal-content rounded-0">
                    <div class="modal-header rounded-0">
                        <h5 class="modal-title" id="exampleModalLabel1">แก้ไขข้อมูลอุปกรณ์</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div id="view">
...
<script>
    var page = "{{$page_url}}/datatable";
        ...
        $('#insert_equipments').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                text: 'คุณต้องการเพิ่มอุปกรณ์หรือไม่?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    let form = document.getElementById('insert_equipments');
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response == true){
                                $('#insert_equipments')[0].reset();
                                Swal.fire('เพิ่มอุปกรณ์เรียบร้อยแล้ว', '', 'success');
                                $('#addModal').modal('hide');
                                loadData(page);
                            }
                        },
        ...
        $('#update_equipments').on('submit', function(event) {
            event.preventDefault(); // ป้องกันการส่งฟอร์มปกติ
            if(!this.checkValidity()) {
                // ถ้าฟอร์มไม่ถูกต้อง
                this.reportValidity();
            ...
            Swal.fire({
                title: 'ยืนยันการดำเนินการ?',
                text: 'คุณต้องการแก้ไขอุปกรณ์หรือไม่?',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'ตกลง',
                cancelButtonText: 'ยกเลิก',
                ...
                if (result.isConfirmed) {
                    ...
                    let form = document.getElementById('update_equipments');
                    let formData = new FormData(form);
                    formData.append('_token', '{{ csrf_token() }}'); // สำหรับ Laravel CSRF
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response == true){
                                Swal.fire('แก้ไขอุปกรณ์เรียบร้อยแล้ว', '', 'success');
        ...
        $('#update_equipment_stocks').on('submit', function(event) {
            ...
            Swal.fire({
                title: 'ยืนยันการดำเนินการ?',
                text: 'คุณต้องการนำเข้าอุปกรณ์หรือไม่?',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'ตกลง',
                cancelButtonText: 'ยกเลิก',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response == true){
                                Swal.fire('นำเข้าอุปกรณ์เรียบร้อยแล้ว', '', 'success');
                                $('#addStock').modal('hide');
                                loadData(page);
                            }
                        },
        ...
        function Delete(id){
                Swal.fire({
                    title: 'ยืนยันการดำเนินการ?',
                    text: 'คุณต้องการลบอุปกรณ์หรือไม่?',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: 'ตกลง',
                    cancelButtonText: 'ยกเลิก',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(response) {
                                if(response == true){
                                    loadData(page);
                                    Swal.fire('ลบอุปกรณ์เรียบร้อยแล้ว', '', 'success');
                                }else{
