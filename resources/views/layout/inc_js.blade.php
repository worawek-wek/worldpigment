    ...
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    {{-- สถานะ "กำลังโหลด" ของตารางรายการ (16/09/2569)                     --}}
    {{--                                                                 --}}
    {{-- server ของลูกค้าดึงข้อมูลช้า — ถ้ากดค้นหา/กรองแล้วจอยังเป็นตาราง   --}}
    {{-- ชุดเดิมเงียบ ๆ ผู้ใช้จะไม่รู้ว่ากดติดหรือไม่ แล้วกดซ้ำ              --}}
    {{--                                                                 --}}
    {{-- วิธีใช้ใน loadData() ของหน้ารายการ:                               --}}
    {{--   showTableLoading();            ← ก่อนยิง ajax                  --}}
    {{--   showTableError();              ← ใน error handler              --}}
    {{-- ⚠ ต้องมี error handler คู่กันเสมอ ไม่งั้น request ที่พังจะทำให้      --}}
    {{--   spinner ค้างถาวร (แย่กว่าเดิมที่ตารางชุดเก่ายังอยู่)              --}}
    {{-- ⚠ ข้าม textStatus === 'abort' — พิมพ์เร็ว ๆ จะ abort request เก่า  --}}
    {{--   ทิ้งเป็นเรื่องปกติ ไม่ใช่ error                                  --}}
    {{--                                                                 --}}
    {{-- scope = selector ของกล่องตาราง (ไม่ระบุ = '#table-data')          --}}
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    <script>
        function showTableLoading(scope){
            $(scope || '#table-data').html(
        ...
        function showTableError(scope, message){
            $(scope || '#table-data').html(
